...
V37: User may wish to set the maximum radial coordinate to twice the light cylinder radius. This requires the light cylinder radius to be calculated first,
...
std::vector< std::vector<double> > B_div                         ( n_points_r, std::vector<double> ( n_points_t ) );
...
void calculate_time_and_rotation_values(){
	// Must be called BEFORE void calculate_gridpoints() if we set bool set_r_max_to_2_R_LC = true.
...
void calculate_gridpoints(){
	...
	if( set_r_max_to_2_R_LC ){
		r_max = 2.0 * R_LC.back();
		delta_r = ( r_max - r_min ) / ( (double) n_points_r - 1.0 );
	}
...
std::vector< std::vector<double> > divergence_from_VSH_coeffs( std::vector< std::vector< std::vector<double> > > &v_VSH_coeffs, std::vector< std::vector< std::vector<double> > > &v_VSH_coeffs_dr ){
	...
	std::vector< std::vector<double> > v_div ( n_points_r, std::vector<double> ( n_points_t ) );
...
void calculate_time_derivatives_of_vector_components(){
	...
	B_div = std::vector< std::vector<double> > ( n_points_r, std::vector<double> ( n_points_t ) );
...
void output_headers_to_csv_profiles(){
	...
	std::cout << "output headers to csv profiles" << std::endl;
}
...
void output_headers_to_csv_history(){
	...
	std::cout << "output headers to csv history" << std::endl;
...
void output_headers_to_csv_VSH_coeffs(){
	...
	std::cout << "output headers to csv VSH coeffs" << std::endl;
...
void output_headers_to_csv_BCs(){
	...
	std::cout << "output headers to csv BCs" << std::endl;
