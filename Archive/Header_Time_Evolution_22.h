...
V22: Rename everything to "fields" and store in a 6-component vector with index f = 0-5 for B_r, B_theta, B_phi, E_r, E_theta, E_phi.
	 ...
	 Similarly, removed nonrotation-specific BCs at the outer boundary because I was forcing B_r,E_r.
*/
	...
	for( int i=0; i<n_points_r; i++ ){
		for( int j=0; j<n_points_t; j++ ){
			...
			if( ( T_index >= T_index_Omega_ramp_start ) and ( T_index <= T_index_Omega_ramp_stop ) ){
				fields_dT[3][i][j] += E_r_dT_function( r[i], t[j], dOmega_by_dT );
				fields_dT[4][i][j] += E_t_dT_function( r[i], t[j], dOmega_by_dT );
				fields_dT[5][i][j] += E_p_dT_function( r[i], t[j], dOmega_by_dT );
			}
...
void apply_inner_boundary_conditions(){
	...
	for( int j=0; j<n_points_t; j++ ){
		...
		if( j == 300 ){
			std::cout       << T_index <<"\t"<< r_min <<"\t"<< t[j] <<"\t"<< B_r_function( r_min, t[j] ) <<"\t"<< Omega <<"\t"<< r[0] <<"\t"<< st[j] <<"\t"<<  B_r_function( r_min, t[j] ) * - Omega * r[0] * st[j] << std::endl;
			output_file_log << T_index <<"\t"<< r_min <<"\t"<< t[j] <<"\t"<< B_r_function( r_min, t[j] ) <<"\t"<< Omega <<"\t"<< r[0] <<"\t"<< st[j] <<"\t"<<  B_r_function( r_min, t[j] ) * - Omega * r[0] * st[j] << "\n";
		}
		fields[5][0][j] = 0;
	}
...
double estimate_csv_profiles_filesize(){
	...
	int n_columns = 9 + 8 + 7 * 6;		// You are responsible for keeping this value updated.
	int n_rows = 0;
...
void output_headers_to_csv_profiles(){
	...
	output_file_profiles << ",B,E,alpha,beta,E_dot_B,B_squared_minus_E_squared,B_r_dr2,magnetic_energy_density,friction";
