...
V31: Change associated Legendre function definitions to exactly match those in the writeup.
     Simplify the numerical integration functions.
	 Add functions void recalculate_B_from_VSH_coeffs() and void recalculate_E_from_VSH_coeffs() to take that calculation outside the stdev function and make it more widely available.
	 Rename void calculate_stdev() -> void calculate_stdev_of_VSH_decomposition().
	 Reorder to better match workflow.
	 Remove option to record in void apply_outer_boundary_conditions(). Unnecessary complication for such a small timesave, so record by default.
*/
...
void calculate_gridpoints(){
	...
	//--- Radial and polar coordinates, plus sin(theta) and cos(theta) ---
...
double integral_trapezium_1D_theta( std::vector<double> f ){
	// 1D integral of f(theta) in polar coordinates, given pre-calculated list of function values.
	
	double ret = 0;
	
	for( int j=0; j<=n_points_t-2; j++ ){
		ret += ( f[j+1] + f[j] ) * finite_line_element_dt[j];
	}
...
double integral_trapezium_2D_r_theta( std::vector< std::vector<double> > f ){
	...
	for( int i=0; i<=n_points_r-2; i++ ){
		for( int j=0; j<=n_points_t-2; j++ ){
			ret += ( f[i][j] + f[i][j+1] + f[i+1][j] + f[i+1][j+1] ) * finite_area_element_for_r_t[i][j];
		}
	}
...
void VSH_decomposition_for_E(){
	...
	for( int i=0; i<n_points_r; i++ ){
		...
		for( int ell=0; ell<=ell_max; ell++ ){
			...
			E_VSH_coeffs[0][i][ell] = integral_trapezium_1D_theta( VSH_integrand_r ) * sqrt_2Lplus1_pi[ell];
		...
		for( int ell=1; ell<=ell_max; ell++ ){
			...
			E_VSH_coeffs[1][i][ell] = integral_trapezium_1D_theta( VSH_integrand_1 ) * sqrt_2Lplus1_pi_over_L_Lplus1[ell];
			E_VSH_coeffs[2][i][ell] = integral_trapezium_1D_theta( VSH_integrand_2 ) * sqrt_2Lplus1_pi_over_L_Lplus1[ell];
...
void VSH_decomposition_for_B(){
	...
	for( int i=0; i<n_points_r; i++ ){
		...
		for( int ell=0; ell<=ell_max; ell++ ){
			...
			B_VSH_coeffs[0][i][ell] = integral_trapezium_1D_theta( VSH_integrand_r ) * sqrt_2Lplus1_pi[ell];
		...
		for( int ell=1; ell<=ell_max; ell++ ){
			...
			B_VSH_coeffs[2][i][ell] = integral_trapezium_1D_theta( VSH_integrand_2 ) * sqrt_2Lplus1_pi_over_L_Lplus1[ell];
...
void recalculate_B_from_VSH_coeffs(){
	...
	for( int i=0; i<n_points_r; i++ ){
		for( int j=0; j<n_points_t; j++ ){
			...
			for( int ell=0; ell<=ell_max; ell++ ){
				B_from_VSH_coeffs[0][i][j] += B_VSH_coeffs[0][i][ell] * sqrt_2Lplus1_over_4pi[ell] * P0[j][ell];
			}
			
			//--- The (1)- and (2)-coefficients start from ell=1 ---
			for( int ell=1; ell<=ell_max; ell++ ){
				B_from_VSH_coeffs[1][i][j] += B_VSH_coeffs[1][i][ell] * sqrt_2Lplus1_over_4pi[ell] * P1[j][ell];
				B_from_VSH_coeffs[2][i][j] += B_VSH_coeffs[2][i][ell] * sqrt_2Lplus1_over_4pi[ell] * P1[j][ell];
			}
...
void recalculate_E_from_VSH_coeffs(){
	...
	for( int i=0; i<n_points_r; i++ ){
		for( int j=0; j<n_points_t; j++ ){
			...
			//--- The r-coefficients start from ell=0 ---
			for( int ell=0; ell<=ell_max; ell++ ){
				E_from_VSH_coeffs[0][i][j] += E_VSH_coeffs[0][i][ell] * sqrt_2Lplus1_over_4pi[ell] * P0[j][ell];
			}
			
			//--- The (1)- and (2)-coefficients start from ell=1 ---
			for( int ell=1; ell<=ell_max; ell++ ){
				E_from_VSH_coeffs[1][i][j] += E_VSH_coeffs[1][i][ell] * sqrt_2Lplus1_over_4pi[ell] * P1[j][ell];
				E_from_VSH_coeffs[2][i][j] += E_VSH_coeffs[2][i][ell] * sqrt_2Lplus1_over_4pi[ell] * P1[j][ell];
			}
...
void calculate_magnetic_energy(){
	...
	total_magnetic_energy = 2.0 * pi * integral_trapezium_2D_r_theta( magnetic_energy_density );
