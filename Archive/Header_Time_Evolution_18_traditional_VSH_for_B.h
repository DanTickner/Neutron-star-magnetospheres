/*
Header_Time_Evolution_18_traditional_VSH_for_B.h
...
V18: Calculate B using the "normal VSH decomposition" instead of taking advantage of its divergencelessness. This gives more immediate accuracy; we may find issues at later times if div(B) grows without limit.
*/
...
std::vector< std::vector<double> > div_B        ( n_points_r, std::vector<double> ( n_points_t ) );
...
double time_taken_before_ramp = 0;			// Used in estimates of runtime remaining because we have different ell_max before and after the ramp onset to improve code accuracy.
...
void VSH_decomposition_for_E( int n ){
	...
	for( int i=0; i<n_points_r; i++ ){
		...
		for( int ell=0; ell<=ell_max_for_this_timestep; ell++ ){
			...
			for( int j=0; j<n_points_t; j++ ){
				VSH_integrand_r[j] = E_r[n][i][j] * P0[j][ell];
			}
			
			E_r_L[i][ell] = integral_trapezium_sintheta_dtheta( VSH_integrand_r ) * sqrt( (2.0*ell+1.0)*pi );
		...
		for( int ell=1; ell<=ell_max_for_this_timestep; ell++ ){
			...
			for( int j=0; j<n_points_t; j++ ){
				VSH_integrand_1[j] = E_t[n][i][j] * P1[j][ell];
				VSH_integrand_2[j] = E_p[n][i][j] * P1[j][ell];
			}
			
			E_1_L[i][ell] = integral_trapezium_sintheta_dtheta( VSH_integrand_1 ) * sqrt( (2.0*ell+1.0)*pi ) / ( (double)ell*(ell+1.0) );
			E_2_L[i][ell] = integral_trapezium_sintheta_dtheta( VSH_integrand_2 ) * sqrt( (2.0*ell+1.0)*pi ) / ( (double)ell*(ell+1.0) );
...
void VSH_decomposition_for_B( int n ){
	...
	for( int i=0; i<n_points_r; i++ ){
		...
		for( int ell=0; ell<=ell_max_for_this_timestep; ell++ ){
			...
			for( int j=0; j<n_points_t; j++ ){
				VSH_integrand_r[j] = B_r[n][i][j] * P0[j][ell];
			}
			
			B_r_L[i][ell] = integral_trapezium_sintheta_dtheta( VSH_integrand_r ) * sqrt( (2.0*ell+1.0)*pi );
		...
		for( int ell=1; ell<=ell_max_for_this_timestep; ell++ ){
			...
			for( int j=0; j<n_points_t; j++ ){
				VSH_integrand_1[j] = B_t[n][i][j] * P1[j][ell];
				VSH_integrand_2[j] = B_p[n][i][j] * P1[j][ell];
			}
			
			B_1_L[i][ell] = integral_trapezium_sintheta_dtheta( VSH_integrand_1 ) * sqrt( (2.0*ell+1.0)*pi ) / ( (double)ell*(ell+1.0) );
			B_2_L[i][ell] = integral_trapezium_sintheta_dtheta( VSH_integrand_2 ) * sqrt( (2.0*ell+1.0)*pi ) / ( (double)ell*(ell+1.0) );
	...
	for( int i=0; i<n_points_r; i++ ){
		for( int j=0; j<n_points_t; j++ ){
			...
			for( int L=1; L<=ell_max_for_this_timestep; L++ ){
				...
				div_B[i][j] += sqrt_2Lplus1_over_4pi[L] * ( B_r_L_dr[i][L] + 2.0*pow(r[i],-1) * B_r_L[i][L] - L*(L+1.0)*pow(r[i],-1) * B_1_L[i][L] );
...
void calculate_radial_derivatives_of_VSH_coeffs(){
	...
	for( int L=0; L<=ell_max_for_this_timestep; L++ ){
		...
		B_1_L_dr [0][L] = ( B_1_L[1][L] - B_1_L[0][L] ) / delta_r;
		...
		for( int i=1; i<n_points_r-1; i++ ){
			...
			B_1_L_dr [i][L] = ( B_1_L[i+1][L] - B_1_L[i-1][L] ) / ( 2.0*delta_r );
		...
		B_1_L_dr .back()[L] = ( B_1_L.back()[L] - B_1_L[n_points_r-2][L] ) / delta_r;
...
void output_headers_to_csv_profiles(){
	...
	output_file_profiles << ",friction,div_B";
