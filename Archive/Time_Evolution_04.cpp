...
g++ Time_Evolution_04.cpp -o Time_Evolution_04
Time_Evolution_04
...
V04: Begin with zero rotation (i.e. zero magnetic field). Every n=10 timesteps, add an electric field with 10% rotation to represent a slow ramp-up.
...
std::string output_filename = "CSV/20230818_ramp_Omega.csv";
...
double Omega_T = 0;	// Value of Omega at time T;
double delta_Omega = Omega / 10.0;

#include "Headers/Header_Time_Evolution_04.h"
...
int main(){
	...
	// For example, we don't do a VSH decomposition because we already have exact expressions for the coefficients.
	// Also, we don't apply boundary conditions until there is a nonzero electric field.
	...
	for( int T_index=1; T_index<=n_timesteps; T_index++ ){
		...
		if( ( T_index % 10 == 0 ) and ( Omega_T <= Omega ) ){
			Omega_T += delta_Omega;
			
			for( int i=0; i<n_points_r; i++ ){
				for( int j=0; j<n_points_t; j++ ){
					E_r[i][j] += delta_Omega * pow( r[i], -2 ) * pow( st[j], 2 );
					E_t[i][j] += delta_Omega * -2.0 * pow( r[i], -2 ) * st[j] * ct[j];
				}
			}
			
		}
		
		std::cout << T_index <<"\t"<< Omega_T <<"\t"<< Omega << std::endl;
