...
V08: Introduce the frictional term sigma_s in Parfrey 2012, PhD thesis, \S3.9, Eqs. (3.39) and (3.40).
     Variable is named friction.
	 Three free parameters which are just guessed. It doesn't seem the way to go. See OneNote 20230911.
*/
...
void calculate_time_derivatives(){
	for( int i=0; i<n_points_r; i++ ){
		for( int j=0; j<n_points_t; j++ ){
			...
			double friction = 0;
			if( r[i] >= r_sponge ){
				double sigma_0 = 1.0e-6;
				double gamma   = 6.0;
				double beta    = 4.0;
				friction = pow( ( r[i] - r_sponge ) / ( r_max - r_sponge ), beta );
				friction = sigma_0 * ( 1.0 - exp( - gamma * friction ) );
			}
			
			//--- Time derivatives ---
			B_r_dT[i][j] = - curl_E_r;
			B_t_dT[i][j] = - curl_E_t - friction;
			B_p_dT[i][j] = - curl_E_p - friction;
			
			E_r_dT[i][j] = curl_B_r + alpha[i][j] * B_r[i][j] + beta[i][j] * E_cross_B_r - friction * E_r[i]j] // check;
			E_r_dT[i][j] = curl_B_t + alpha[i][j] * B_t[i][j] + beta[i][j] * E_cross_B_t - friction;
			E_r_dT[i][j] = curl_B_p + alpha[i][j] * B_p[i][j] + beta[i][j] * E_cross_B_p - friction;
