...
V06: Chebyshev polynomial decomposition instead of finite differencing in the radial direction.
...
std::vector< std::vector<double> > cnR ( n_points_r, std::vector<double> ( n_max+1 ) );
...
std::vector< std::vector<double> > B_r_n_L    ( n_max+1, std::vector<double> ( ell_max+1 ) );
std::vector< std::vector<double> > B_1_n_L    ( n_max+1, std::vector<double> ( ell_max+1 ) );
std::vector< std::vector<double> > B_2_n_L    ( n_max+1, std::vector<double> ( ell_max+1 ) );
std::vector< std::vector<double> > E_r_n_L    ( n_max+1, std::vector<double> ( ell_max+1 ) );
std::vector< std::vector<double> > E_1_n_L    ( n_max+1, std::vector<double> ( ell_max+1 ) );
std::vector< std::vector<double> > E_2_n_L    ( n_max+1, std::vector<double> ( ell_max+1 ) );
...
std::vector<double> B_r_n_L_integrand ( n_points_r );	// Integrands for Chebyshev decomposition.
std::vector<double> B_1_n_L_integrand ( n_points_r );
std::vector<double> B_2_n_L_integrand ( n_points_r );
std::vector<double> E_r_n_L_integrand ( n_points_r );
std::vector<double> E_1_n_L_integrand ( n_points_r );
std::vector<double> E_2_n_L_integrand ( n_points_r );
...
double four_over_r_max_minus_r_min         = 4.0 * pow( r_max-r_min, -1 );
double four_over_r_max_minus_r_min_squared = 4.0 * pow( r_max-r_min, -2 );
...
void calculate_Chebyshev_polynomials(){
	for( int i=0; i<n_points_r; i++ ){
		for( int n=0; n<=n_max; n++ ){
			Tn[i][n] = cos( n * R[i] );	// Equivalent to cos( n * acos( Lambda_inverse[i] ) ).
		}
	}
}
...
void Chebyshev_decomposition(){
	
	//----- L=0 only applies to radial coefficients -----
	
	//--- n=0 ---
	for( int i=0; i<n_points_r; i++ ){
		B_r_n_L_integrand[i] = B_r_L[i][0];
		E_r_n_L_integrand[i] = E_r_L[i][0];
	}
	
	B_r_n_L[0][0] = integral_trapezium( B_r_n_L_integrand, R ) / pi;
	E_r_n_L[0][0] = integral_trapezium( E_r_n_L_integrand, R ) / pi;
	
	//--- n>0 ---
	for( int n=1; n<=n_max; n++ ){
		
		for( int i=0; i<n_points_r; i++ ){
			B_r_n_L_integrand[i] = B_r_L[i][0] * cnR[i][n];
			E_r_n_L_integrand[i] = E_r_L[i][0] * cnR[i][n];
		}
		
		B_r_n_L[n][0] = integral_trapezium( B_r_n_L_integrand, R ) * 2.0 / pi;
		E_r_n_L[n][0] = integral_trapezium( E_r_n_L_integrand, R ) * 2.0 / pi;
	}
	
	
	//----- L>0 applies to all coefficients -----
	for( int L=1; L<=ell_max; L++ ){
		
		//--- n=0 ---
		for( int i=0; i<n_points_r; i++ ){
			B_r_n_L_integrand[i] = B_r_L[i][L];
			B_1_n_L_integrand[i] = B_1_L[i][L];
			B_2_n_L_integrand[i] = B_2_L[i][L];
			E_r_n_L_integrand[i] = E_r_L[i][L];
			E_1_n_L_integrand[i] = E_1_L[i][L];
			E_2_n_L_integrand[i] = E_2_L[i][L];
		}
		
		B_r_n_L[0][L] = integral_trapezium( B_r_n_L_integrand, R ) / pi;
		B_1_n_L[0][L] = integral_trapezium( B_1_n_L_integrand, R ) / pi;
		B_2_n_L[0][L] = integral_trapezium( B_2_n_L_integrand, R ) / pi;
		E_r_n_L[0][L] = integral_trapezium( E_r_n_L_integrand, R ) / pi;
		E_1_n_L[0][L] = integral_trapezium( E_1_n_L_integrand, R ) / pi;
		E_2_n_L[0][L] = integral_trapezium( E_2_n_L_integrand, R ) / pi;
		
		//--- n>0 ---
		for( int n=1; n<=n_max; n++ ){
			
			for( int i=0; i<n_points_r; i++ ){
				B_r_n_L_integrand[i] = B_r_L[i][L] * cnR[i][n];
				B_1_n_L_integrand[i] = B_1_L[i][L] * cnR[i][n];
				B_2_n_L_integrand[i] = B_2_L[i][L] * cnR[i][n];
				E_r_n_L_integrand[i] = E_r_L[i][L] * cnR[i][n];
				E_1_n_L_integrand[i] = E_1_L[i][L] * cnR[i][n];
				E_1_n_L_integrand[i] = E_2_L[i][L] * cnR[i][n];
			}
			
			B_r_n_L[n][L] = integral_trapezium( B_r_n_L_integrand, R ) * 2.0 / pi;
			B_1_n_L[n][L] = integral_trapezium( B_1_n_L_integrand, R ) * 2.0 / pi;
			B_2_n_L[n][L] = integral_trapezium( B_2_n_L_integrand, R ) * 2.0 / pi;
			E_r_n_L[n][L] = integral_trapezium( E_r_n_L_integrand, R ) * 2.0 / pi;
			E_1_n_L[n][L] = integral_trapezium( E_1_n_L_integrand, R ) * 2.0 / pi;
			E_2_n_L[n][L] = integral_trapezium( E_2_n_L_integrand, R ) * 2.0 / pi;
			
			if( ( n <= 5 ) and ( L <= 2 ) ){
				std::cout << n <<"\t"<< L <<"\t|\t"<< B_r_n_L[n][L] <<"\t"<< B_1_n_L[n][L] <<"\t"<< B_2_n_L[n][L] <<"\t|\t"<< E_r_n_L[n][L] <<"\t"<< E_1_n_L[n][L] <<"\t"<< E_2_n_L[n][L] << std::endl;
			}
...
void calculate_radial_derivatives_of_VSH_coeffs_from_Chebyshev_series(){
	
	//----- Reset arrays to zero -----
	B_r_L_dr  = std::vector< std::vector<double> > ( n_points_r, std::vector<double> ( ell_max+1 ) );
	B_2_L_dr  = std::vector< std::vector<double> > ( n_points_r, std::vector<double> ( ell_max+1 ) );
	E_r_L_dr  = std::vector< std::vector<double> > ( n_points_r, std::vector<double> ( ell_max+1 ) );
	E_1_L_dr  = std::vector< std::vector<double> > ( n_points_r, std::vector<double> ( ell_max+1 ) );
	E_2_L_dr  = std::vector< std::vector<double> > ( n_points_r, std::vector<double> ( ell_max+1 ) );
	B_r_L_dr2 = std::vector< std::vector<double> > ( n_points_r, std::vector<double> ( ell_max+1 ) );
	...
	for( int i=0; i<n_points_r; i++ ){
		for( int L=0; L<=ell_max; L++ ){
			...
			for( int n=0; n<=n_max; n++ ){
				for( int k=n+1; k<=n_max; k++ ){
					if( (n+k)%2 == 1 ){
						...
						double factor = four_over_r_max_minus_r_min * h_n * k * Tn[i][n];
						B_r_L_dr[i][L] += factor * B_r_n_L[k][L];
						B_2_L_dr[i][L] += factor * B_2_n_L[k][L];
						E_r_L_dr[i][L] += factor * B_r_n_L[k][L];
						E_1_L_dr[i][L] += factor * B_1_n_L[k][L];
						E_2_L_dr[i][L] += factor * B_2_n_L[k][L];
					}
				}
			}
			...
			for( int k=2; k<=n_max; k+=2 ){
				B_r_L_dr2[i][L] += 0.5 * pow(k,3) * B_r_n_L[k][L];
			}
			
			for( int n=1; n<=n_max; n++ ){
				for( int k=n+2; k<=n_max; k+=2 ){
					B_r_L_dr2[i][L] += k * ( k*k - n*n ) * B_r_n_L[k][L] * Tn[i][n];
				}
			}
			
			B_r_L_dr2[i][L] *= four_over_r_max_minus_r_min_squared;
