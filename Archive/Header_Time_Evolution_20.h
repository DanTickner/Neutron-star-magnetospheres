...
V20: T_index is now a global variable because it is needed outside the for-loop in Time_Evolution_xx.cpp. Remove it as a function argument.
     We must apply the boundary conditions at each step, including T=0.
	 This means there's no point having initial VSH coefficients for B, because the boundary conditions will change it.
	 Reorder the "n" index so that n=0 is the current step, n=1 is the previous step and n=n means n steps ago. That is, we are only interested in updating the n=0 values of quantities, and the n>0 values are only to keep a rolling record of previous values.
*/
...
void VSH_decomposition_for_E(){
	...
	for( int i=0; i<n_points_r; i++ ){
		...
		for( int ell=0; ell<=ell_max; ell++ ){
			...
			for( int j=0; j<n_points_t; j++ ){
				VSH_integrand_r[j] = E_r[0][i][j] * P0[j][ell];
			}
		...
		for( int ell=1; ell<=ell_max; ell++ ){
			...
			for( int j=0; j<n_points_t; j++ ){
				VSH_integrand_1[j] = E_t[0][i][j] * P1[j][ell];
				VSH_integrand_2[j] = E_p[0][i][j] * P1[j][ell];
			}
...
void VSH_decomposition_for_B(){
	...
	for( int i=0; i<n_points_r; i++ ){
		...
		for( int ell=0; ell<=ell_max; ell++ ){
			...
			for( int j=0; j<n_points_t; j++ ){
				VSH_integrand_r[j] = B_r[0][i][j] * P0[j][ell];
			}
		...
		for( int ell=1; ell<=ell_max; ell++ ){
			...
			for( int j=0; j<n_points_t; j++ ){
				VSH_integrand_2[j] = B_p[0][i][j] * P1[j][ell];
			}
...
void calculate_Cartesian_vector_components(){
	...
	for( int i=0; i<n_points_r; i++ ){
		for( int j=0; j<n_points_t; j++ ){
			...
			B_x[i][j] = B_r[0][i][j] * st[j] + B_t[0][i][j] * ct[j];
			B_y[i][j] = B_p[0][i][j];									// Extra variable assignment not necessary, but use for now to avoid confusion.
			B_z[i][j] = B_r[0][i][j] * ct[j] - B_t[0][i][j] * st[j];
			
			E_x[i][j] = E_r[0][i][j] * st[j] + E_t[0][i][j] * ct[j];
			E_y[i][j] = E_p[0][i][j];									// Extra variable assignment not necessary, but use for now to avoid confusion.
			E_z[i][j] = E_r[0][i][j] * ct[j] - E_t[0][i][j] * st[j];
	...
	for( int i=0; i<n_points_r; i++ ){
		for( int j=1; j<n_points_t-1; j++ ){
			if( r[i] * pow( st[j], -2 ) < 1.0 / Omega ){
				...
				double E_dot_B = E_r[0][i][j]*B_r[0][i][j] + E_t[0][i][j]*B_t[n][i][j] + E_p[0][i][j]*B_p[0][i][j];
				double Bsq     = B_r[0][i][j]*B_r[0][i][j] + B_t[0][i][j]*B_t[n][i][j] + B_p[0][i][j]*B_p[0][i][j];
				double Esq     = E_r[0][i][j]*E_r[0][i][j] + E_t[0][i][j]*E_t[n][i][j] + E_p[0][i][j]*E_p[0][i][j];
				...
				if( ( E_dot_B > E_dot_B_tol ) or ( Bsq - Esq < 0 ) ){
					...
					double E_prime_r = E_r[0][i][j] - E_dot_B_over_Bsq * B_r[0][i][j];
					double E_prime_t = E_t[0][i][j] - E_dot_B_over_Bsq * B_t[0][i][j];
					double E_prime_p = E_p[0][i][j] - E_dot_B_over_Bsq * B_p[0][i][j];
					double sqrt_Bsq_over_Eprimesq = sqrt( Bsq / ( E_prime_r*E_prime_r + E_prime_t*E_prime_t + E_prime_p*E_prime_p ) );
					
					E_r[0][i][j] = E_prime_r * sqrt_Bsq_over_Eprimesq;
					E_t[0][i][j] = E_prime_t * sqrt_Bsq_over_Eprimesq;
					E_p[0][i][j] = E_prime_p * sqrt_Bsq_over_Eprimesq;
...
void calculate_force_free_conditions_for_CSV_output(){
	// For output to CSV file only.
	for( int i=0; i<n_points_r; i++ ){
		for( int j=0; j<n_points_t; j++ ){
			E_dot_B      [i][j] = E_r[0][i][j]*B_r[0][i][j] + E_t[0][i][j]*B_t[0][i][j] + E_p[0][i][j]*B_p[0][i][j];
			Bsq_minus_Esq[i][j] = B_r[0][i][j]*B_r[0][i][j] + B_t[0][i][j]*B_t[0][i][j] + B_p[0][i][j]*B_p[0][i][j] - E_r[0][i][j]*E_r[0][i][j] - E_t[0][i][j]*E_t[0][i][j] - E_p[0][i][j]*E_p[0][i][j];
		}
	}
}
...
void calculate_time_derivatives_of_vector_components(){
	...
	for( int i=0; i<n_points_r; i++ ){
		for( int j=0; j<n_points_t; j++ ){
			...
			//--- alpha and beta ---
			double B_dot_curl_B = B_r[0][i][j]*curl_B_r   [i][j] + B_t[0][i][j]*curl_B_t   [i][j] + B_p[0][i][j]*curl_B_p   [i][j];
			double E_dot_curl_E = E_r[0][i][j]*curl_E_r   [i][j] + E_t[0][i][j]*curl_E_t   [i][j] + E_p[0][i][j]*curl_E_p   [i][j];
			double B_squared    = B_r[0][i][j]*B_r     [0][i][j] + B_t[0][i][j]*     B_t[0][i][j] + B_p[0][i][j]*     B_p[0][i][j];
			double E_squared    = E_r[0][i][j]*E_r     [0][i][j] + E_t[0][i][j]*     E_t[0][i][j] + E_p[0][i][j]*     E_p[0][i][j];
			
			alpha[i][j] = ( E_dot_curl_E - B_dot_curl_B ) / B_squared;
			beta [i][j] = - div_E[i][j] / B_squared;
			
			//--- E cross B ---
			double E_cross_B_r = E_t[0][i][j] * B_p[0][i][j] - E_p[0][i][j] * B_t[0][i][j];
			double E_cross_B_t = E_p[0][i][j] * B_r[0][i][j] - E_r[0][i][j] * B_p[0][i][j];
			double E_cross_B_p = E_r[0][i][j] * B_t[0][i][j] - E_t[0][i][j] * B_r[0][i][j];
			...
			B_r_dT[0][i][j] = - curl_E_r[i][j];
			B_t_dT[0][i][j] = - curl_E_t[i][j] - friction[i][j] * B_t[0][i][j];
			B_p_dT[0][i][j] = - curl_E_p[i][j] - friction[i][j] * B_p[0][i][j];
			
			E_r_dT[0][i][j] = curl_B_r[i][j] + alpha[i][j] * B_r[0][i][j] + beta[i][j] * E_cross_B_r - friction[i][j] * E_r[0][i][j];
			E_t_dT[0][i][j] = curl_B_t[i][j] + alpha[i][j] * B_t[0][i][j] + beta[i][j] * E_cross_B_t - friction[i][j] * E_t[0][i][j];
			E_p_dT[0][i][j] = curl_B_p[i][j] + alpha[i][j] * B_p[0][i][j] + beta[i][j] * E_cross_B_p - friction[i][j] * E_p[0][i][j];
			
			if( ( T_index >= T_index_Omega_ramp_start ) and ( T_index <= T_index_Omega_ramp_stop ) ){
				E_r_dT[0][i][j] += E_r_dT_function( r[i], t[j], dOmega_by_dT );
				E_t_dT[0][i][j] += E_t_dT_function( r[i], t[j], dOmega_by_dT );
				E_p_dT[0][i][j] += E_p_dT_function( r[i], t[j], dOmega_by_dT );
			}
...
void integrate_vectors_wrt_time_Adams_Bashforth_Order_1(){
	...
	for( int i=0; i<n_points_t; i++ ){
		for( int j=0; j<n_points_t; j++ ){
			...
			E_t   [1][i][j] = E_t   [0][i][j];
			E_p   [1][i][j] = E_p   [0][i][j];
			B_r_dT[1][i][j] = B_r_dT[0][i][j];
			B_t_dT[1][i][j] = B_t_dT[0][i][j];
			B_p_dT[1][i][j] = B_p_dT[0][i][j];
			E_r_dT[1][i][j] = E_r_dT[0][i][j];
			E_t_dT[1][i][j] = E_t_dT[0][i][j];
			E_p_dT[1][i][j] = E_p_dT[0][i][j];
			
			//--- Calculate the values at the current timestep by Adams-Bashforth integration ---
			B_r[0][i][j] = B_r[1][i][j] + delta_T * B_r_dT[1][i][j];
			B_t[0][i][j] = B_t[1][i][j] + delta_T * B_t_dT[1][i][j];
			B_p[0][i][j] = B_p[1][i][j] + delta_T * B_p_dT[1][i][j];
			E_r[0][i][j] = E_r[1][i][j] + delta_T * E_r_dT[1][i][j];
			E_t[0][i][j] = E_t[1][i][j] + delta_T * E_t_dT[1][i][j];
			E_p[0][i][j] = E_p[1][i][j] + delta_T * E_p_dT[1][i][j];
...
void integrate_vectors_wrt_time_Adams_Bashforth_Order_2(){
	...
	for( int i=0; i<n_points_t; i++ ){
		for( int j=0; j<n_points_t; j++ ){
			...
			E_t   [2][i][j] = E_t   [1][i][j];
			E_p   [2][i][j] = E_p   [1][i][j];
			B_r_dT[2][i][j] = B_r_dT[1][i][j];
			B_t_dT[2][i][j] = B_t_dT[1][i][j];
			B_p_dT[2][i][j] = B_p_dT[1][i][j];
			E_r_dT[2][i][j] = E_r_dT[1][i][j];
			E_t_dT[2][i][j] = E_t_dT[1][i][j];
			E_p_dT[2][i][j] = E_p_dT[1][i][j];
			...
			//--- Calculate the values at the current timestep by Adams-Bashforth integration ---
			B_r[0][i][j] = B_r[1][i][j] + delta_T * ( 1.5 * B_r_dT[1][i][j] - 0.5 * B_r_dT[2][i][j] );
			B_t[0][i][j] = B_t[1][i][j] + delta_T * ( 1.5 * B_t_dT[1][i][j] - 0.5 * B_t_dT[2][i][j] );
			B_p[0][i][j] = B_p[1][i][j] + delta_T * ( 1.5 * B_p_dT[1][i][j] - 0.5 * B_p_dT[2][i][j] );
			E_r[0][i][j] = E_r[1][i][j] + delta_T * ( 1.5 * E_r_dT[1][i][j] - 0.5 * E_r_dT[2][i][j] );
			E_t[0][i][j] = E_t[1][i][j] + delta_T * ( 1.5 * E_t_dT[1][i][j] - 0.5 * E_t_dT[2][i][j] );
			E_p[0][i][j] = E_p[1][i][j] + delta_T * ( 1.5 * E_p_dT[1][i][j] - 0.5 * E_p_dT[2][i][j] );
...
void apply_inner_boundary_conditions(){
	...
	for( int j=0; j<n_points_t; j++ ){
		B_r_inner_before_BCs[j] = B_r[0][0][j];
		B_t_inner_before_BCs[j] = B_t[0][0][j];
		B_p_inner_before_BCs[j] = B_p[0][0][j];
		E_r_inner_before_BCs[j] = E_r[0][0][j];
		E_t_inner_before_BCs[j] = E_t[0][0][j];
		E_p_inner_before_BCs[j] = E_p[0][0][j];
	}
	
	//--- Apply BCs ---
	if( T_index < T_index_Omega_ramp_start ){
		for( int j=0; j<n_points_t; j++ ){
			B_r[0][0][j] = B_r_function( r_min, t[j] );
			B_t[0][0][j] = B_t_function( r_min, t[j] );
			B_p[0][0][j] = B_p_function( r_min, t[j] );
			E_r[0][0][j] = 0;
			E_p[0][0][j] = 0;
			E_t[0][0][j] = 0;
		}
		...
		for( int j=0; j<n_points_t; j++ ){
			B_r[0][0][j] = B_r_function( r_min, t[j] );
			E_t[0][0][j] = - Omega * r[0] * st[j] * B_r_function( r_min, t[j] );
			E_p[0][0][j] = 0;
		}
	}
	...
	for( int j=0; j<n_points_t; j++ ){
		B_r_inner_after_BCs[j] = B_r[0][0][j];
		B_t_inner_after_BCs[j] = B_t[0][0][j];
		B_p_inner_after_BCs[j] = B_p[0][0][j];
		E_r_inner_after_BCs[j] = E_r[0][0][j];
		E_t_inner_after_BCs[j] = E_t[0][0][j];
		E_p_inner_after_BCs[j] = E_p[0][0][j];
	}
...
void apply_outer_boundary_conditions(){
	...
	//--- Record values before BCs applied ---
	for( int j=0; j<n_points_t; j++ ){
		B_r_outer_before_BCs[j] = B_r[0][0][j];
		B_t_outer_before_BCs[j] = B_t[0][0][j];
		B_p_outer_before_BCs[j] = B_p[0][0][j];
		E_r_outer_before_BCs[j] = E_r[0][0][j];
		E_t_outer_before_BCs[j] = E_t[0][0][j];
		E_p_outer_before_BCs[j] = E_p[0][0][j];
	}
	
	//--- Apply BCs ---
	if( T_index < T_index_Omega_ramp_start ){
		for( int j=0; j<n_points_t; j++ ){
			B_r[0].back()[j] = B_r_function( r_max, t[j] );
			B_t[0].back()[j] = B_t_function( r_max, t[j] );
			B_p[0].back()[j] = B_p_function( r_max, t[j] );
			E_r[0].back()[j] = 0;
			E_p[0].back()[j] = 0;
			E_t[0].back()[j] = 0;
		}
		...
		for( int j=0; j<n_points_t; j++ ){
			B_t[0].back()[j] = 0.5 * ( B_t[0].back()[j] - E_p[0].back()[j] );
			B_p[0].back()[j] = 0.5 * ( E_t[0].back()[j] + B_p[0].back()[j] );
			E_t[0].back()[j] =   B_p[0].back()[j];
			E_p[0].back()[j] = - B_t[0].back()[j];
		}
	}
	
	//--- Record values after BCs applied ---
	for( int j=0; j<n_points_t; j++ ){
		B_r_outer_after_BCs[j] = B_r[0][0][j];
		B_t_outer_after_BCs[j] = B_t[0][0][j];
		B_p_outer_after_BCs[j] = B_p[0][0][j];
		E_r_outer_after_BCs[j] = E_r[0][0][j];
		E_t_outer_after_BCs[j] = E_t[0][0][j];
		E_p_outer_after_BCs[j] = E_p[0][0][j];
	}
...
void calculate_magnetic_energy(){
	...
	for( int i=0; i<n_points_r; i++ ){
		for( int j=0; j<n_points_t; j++ ){
			magnetic_energy_density[i][j] = pow(B_r[0][i][j],2) + pow(B_t[0][i][j],2) + pow(B_p[0][i][j],2);
		}
	}
...
double estimate_csv_profiles_filesize(){
	...
	int n_columns = 64;		// You are responsible for keeping this value updated.
	int n_rows = 0;
		...
		for( int i=csv_profiles_write_i_min; i<n_points_r; i++ ){
			for( int j=0; j<n_points_t; j++ ){
				...
				if( condition_r and condition_t ){
					...
					double B = sqrt( pow( B_r[0][i][j], 2 ) + pow( B_t[0][i][j], 2 ) + pow( B_p[0][i][j], 2 ) );
					double E = sqrt( pow( E_r[0][i][j], 2 ) + pow( E_t[0][i][j], 2 ) + pow( E_p[0][i][j], 2 ) );
					...
					if( abs( B_r[0][i][j] ) > tolerance_for_zero ){ B_r_dT_over_B_r = B_r_dT[0][i][j] / B_r[0][i][j]; }
					if( abs( B_t[0][i][j] ) > tolerance_for_zero ){ B_t_dT_over_B_t = B_t_dT[0][i][j] / B_t[0][i][j]; }
					if( abs( B_p[0][i][j] ) > tolerance_for_zero ){ B_p_dT_over_B_p = B_p_dT[0][i][j] / B_p[0][i][j]; }
					if( abs( E_r[0][i][j] ) > tolerance_for_zero ){ E_r_dT_over_E_r = E_r_dT[0][i][j] / E_r[0][i][j]; }
					if( abs( E_t[0][i][j] ) > tolerance_for_zero ){ E_t_dT_over_E_t = E_t_dT[0][i][j] / E_t[0][i][j]; }
					if( abs( E_p[0][i][j] ) > tolerance_for_zero ){ E_p_dT_over_E_p = E_p_dT[0][i][j] / E_p[0][i][j]; }
					...
					output_file_profiles <<std::setprecision(csv_precision)<< T_index <<","<< T <<","<< T*T_factor <<","<< i <<","<< j <<","<< r[i] <<","<< t[j] <<","<< x[i][j] <<","<< z[i][j]
																		   ...
																		   <<","<< B_r    [0][i][j] <<","<< B_t    [0][i][j] <<","<< B_p    [0][i][j] <<","<< E_r    [0][i][j] <<","<< E_t    [0][i][j] <<","<< E_p    [0][i][j]
																		   ...
																		   <<","<< B_r_dT[0][i][j] <<","<< B_t_dT[0][i][j] <<","<< B_p_dT[0][i][j] <<","<< E_r_dT[0][i][j] <<","<< E_t_dT[0][i][j] <<","<< E_p_dT[0][i][j]
...
void count_nans(){
	...
	for( int i=0; i<n_points_r; i++ ){
		for( int j=0; j<n_points_t; j++ ){
			if( ( isnan( B_r[0][i][j] ) ) or ( isinf( B_r[0][i][j] ) ) ){ nans_B_r += 1; }
			if( ( isnan( B_t[0][i][j] ) ) or ( isinf( B_t[0][i][j] ) ) ){ nans_B_t += 1; }
			if( ( isnan( B_p[0][i][j] ) ) or ( isinf( B_p[0][i][j] ) ) ){ nans_B_p += 1; }
			if( ( isnan( E_r[0][i][j] ) ) or ( isinf( E_r[0][i][j] ) ) ){ nans_E_r += 1; }
			if( ( isnan( E_t[0][i][j] ) ) or ( isinf( E_t[0][i][j] ) ) ){ nans_E_t += 1; }
			if( ( isnan( E_p[0][i][j] ) ) or ( isinf( E_p[0][i][j] ) ) ){ nans_E_p += 1; }
		}
	}
