...
V23: Modify the added dE/dT during rotation to include the actual values of B_r and B_t that have evolved so far, not the exact expressions for a dipole. See OneNote notes 20231018Wed.
     Add were_FF_conditions_applied[i][j] to profiles CSV, to give a map of where the force-free conditions are being violated and hence applied.
*/
...
void apply_force_free_conditions_within_force_free_region(){
	...
	if( n_gridpoints_within_force_free_region ){
		percentage_E_points_points_changed_for_second_force_free_condition = 0;
	} else {
	...
	for( int i=0; i<n_points_r; i++ ){
		for( int j=0; j<n_points_t; j++ ){
			...
			if( ( T_index >= T_index_Omega_ramp_start ) and ( T_index <= T_index_Omega_ramp_stop ) ){
				//--- V1 ---
				//fields_dT[3][i][j] += dOmega_by_dT * r[i] * st[j] * fields[1][i][j];
				//fields_dT[4][i][j] -= dOmega_by_dT * r[i] * st[j] * fields[0][i][j];
				
				//--- V2 (product rule) ---
				fields_dT[3][i][j] += r[i] * st[j] * ( dOmega_by_dT * fields[1][i][j] + Omega * fields_dT[1][i][j] );
				fields_dT[4][i][j] -= r[i] * st[j] * ( dOmega_by_dT * fields[0][i][j] + Omega * fields_dT[0][i][j] );
			}
