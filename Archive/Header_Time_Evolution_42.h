...
V42: Remove option to set r_max = 2*R_LC, in order to fully disentangle the gridpoints and the time values.
	 ...
	 Output the field values at (cout_i,cout_j) to the history.csv for easy reference and for plotting.
	 void calculate_dot_products(): Use for-loop to sum over the coordinates to reduce risk of typo.
*/
...
void calculate_functions_of_gridpoints(){
	...
	//--- Chebyshev integration variable R ---
	for( int i=0; i<n_r; i++ ){
...
double integral_Booles_rule( std::vector<double> f, double h ){
	...
	if( N % 4 != 1 ){
		std::cout << "This method is most accurate when you have 4m+1 data points. Consider a different method or adding one or two extra points." << std::endl;
	}
...
std::vector<double> radial_derivatives_1_FD( std::vector<double> &f ){
	...
	//----- Order-h^2 symmetric -----
	/*
	f_dr[0    ] = ( -f[2    ] + 4.0*f[1    ] - 3.0*f[0    ] ) / ( 2.0*delta_r );
	f_dr[n_r-1] = (  f[n_r-3] - 4.0*f[n_r-2] + 3.0*f[n_r-1] ) / ( 2.0*delta_r );
	
	for( int i=1; i<=n_r-2; i++ ){
		f_dr[i] = ( f[i+1] - f[i-1] ) / ( 2.0*delta_r );
	}
	*/
	...
	//----- Order-h^6 symmetric for intermediate points, then decreasing symmetric methods until two-point (order-h) at the boundaries -----
	/*
	f_dr[0    ] = ( f[1    ] - f[0    ] ) / delta_r;
	f_dr[n_r-1] = ( f[n_r-1] - f[n_r-2] ) / delta_r;
	
	f_dr[1    ] = ( f[2    ] - f[0    ] ) / ( 2.0*delta_r );
	f_dr[n_r-2] = ( f[n_r-1] - f[n_r-3] ) / ( 2.0*delta_r );
	
	f_dr[2    ] = ( -f[4    ]+f[0    ] + 8.0*(f[3    ]-f[1    ])  ) / ( 12.0*delta_r );
	f_dr[n_r-3] = ( -f[n_r-1]+f[n_r-5] + 8.0*(f[n_r-2]-f[n_r-4])  ) / ( 12.0*delta_r );
	...
	for( int i=0; i<n_r; i++ ){
		for( int j=0; j<n_t; j++ ){
			...
			if( use_outer_sponge_layer and ( r[i] >= r_sponge ) ){
				...
				/*
				B_dT[1][i][j] *= 1.0 - friction[i];
				B_dT[2][i][j] *= 1.0 - friction[i];
				E_dT[0][i][j] *= 1.0 - friction[i];
				E_dT[1][i][j] *= 1.0 - friction[i];
				E_dT[2][i][j] *= 1.0 - friction[i];
				*/
				...
				/* ~20240527
				B_dT[1][i][j] -= ( B_dT[1][i][j] > 0 ? 1.0 : -1.0 ) * abs( friction[i] * B[1][i][j] );
				B_dT[2][i][j] -= ( B_dT[2][i][j] > 0 ? 1.0 : -1.0 ) * abs( friction[i] * B[2][i][j] );
				E_dT[0][i][j] -= ( E_dT[0][i][j] > 0 ? 1.0 : -1.0 ) * abs( friction[i] * E[0][i][j] );
				E_dT[1][i][j] -= ( E_dT[1][i][j] > 0 ? 1.0 : -1.0 ) * abs( friction[i] * E[1][i][j] );
				E_dT[2][i][j] -= ( E_dT[2][i][j] > 0 ? 1.0 : -1.0 ) * abs( friction[i] * E[2][i][j] );
				*/
