...
V09: UPDATED FROM V07, NOT V08.
     Put associated Legendre functions into two separate for-loops, to remove confusion over why one has three initial terms and the other has two.
	 Make void VSH_decomposition() a single function which takes arguments of the vectors, instead of two standalone functions, for minimising of potential errors at cost of micro-optimised performance.
	 FIX TYPOS referring to A_t twice instead of A_t then A_p in void evaluate_VSH_series() and void evaluate_radial_derivatives().
*/
