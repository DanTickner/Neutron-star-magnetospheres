...
V14: Include functionality for second-order Adams-Bashforth integration.
	 ...
	 Petri's boundary conditions only make sense if the star is rotating. Change the boundary condition functions to maintain the original magnetic field and zero electric field if the star is not yet rotating, and to use Petri's conditions if the star is rotating.
	 Add variable ell_max = ( ell_max_rotation_off or ell_max_rotation_on ) to make the initial no-rotation phase more stable.
	 Instead of checking T_index < T_index_Omega_ramp_start in every function and needing T_index as a function argument, just make a new global variable is_rotation_on, which is set during each timestep in Time_Evolution_nn.cpp, and check this within each function.
*/
