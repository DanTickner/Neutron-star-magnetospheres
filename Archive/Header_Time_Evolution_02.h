...
V02: Redo the VSH series decomposition at every timestep. It is fast, so there is no need to avoid it.
...
void VSH_decomposition_for_B(){
	for( int i=0; i<n_points_r; i++ ){
		for( int ell=0; ell<=ell_max; ell++ ){
			...
			//--- Integrate and put values into array ---
			int j = 3;
			std::cout << B_r[i][j] <<"\t"<< VSH_integrand_r[j] << std::endl;
			std::cout << i <<"\t"<< ell <<"\t|\t"<< std::left<<std::setw(w)<< B_r_L[i][ell] <<std::left<<std::setw(w)<< B_r_L_function(r[i],ell) <<"\t|\t"<<std::left<<std::setw(w)<< B_1_L[i][ell] <<std::left<<std::setw(w)<< B_1_L_function(r[i],ell) <<"\t|\t"<< std::left<<std::setw(w)<< B_2_L[i][ell] <<std::left<<std::setw(w)<< B_2_L_function(r[i],ell) << std::endl;
...
void apply_second_force_free_condition_within_light_cylinder(){
	...
	//--- If updates were made at any points, we must redo the VSH decomposition ---
	if( E_updated ){
		VSH_decomposition_for_E();
	}
	
	//--- Recalculate E dot B and B^2-E^2 at all points (mostly for CSV output) ---
	for( int i=0; i<n_points_r; i++ ){
		for( int j=0; j<n_points_t; j++ ){
