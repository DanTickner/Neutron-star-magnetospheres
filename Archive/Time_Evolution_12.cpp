...
g++ Time_Evolution_12.cpp -o Time_Evolution_12 --std=c++11
Time_Evolution_12
...
V12: Change radial derivatives to use ghost points. This requires a new function call, void set_field_values_at_ghost_points_for_E(). I am not convinced by this method, especially after rotation begins.
     Don't apply the FF condition at the first timestep because we know it's true everywhere for a dipole and it requires us generally to do a VSH decomposition, losing accuracy immediately. However, we must call void calculate_n_gridpoints_within_force_free_region() so that int n_gridpoints_within_force_free_region has an initial value to output.
...
#include "Headers/Header_Time_Evolution_14.h"
...
int main(){
	...
	set_field_values_at_ghost_points_for_E();
	...
	for( int T_index=1; T_index<=n_timesteps; T_index++ ){
		...
		VSH_decomposition( n, B_r, B_t, B_p, B_r_L, B_1_L, B_2_L );
		VSH_decomposition( n, E_r, E_t, E_p, E_r_L, E_1_L, E_2_L );
		calculate_stdev( n );
		...
		apply_inner_boundary_conditions( T_index, n );
