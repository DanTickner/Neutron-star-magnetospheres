...
V17: Revert to the one-side derivatives.
     Two versions of the VSH decomposition function.
	 One for divergence-free fields, which doesn't calculate A^{(1),L} and instead just sets A^{(1),L} to the expresion in terms of A^{r,L}, obviating the function void calculate_B_1_L_and_B_1_L_dr().
	 The other is for general fields and calculates all three sets of coefficients.
	 Because the first is only used for B, and the second only for E, we may as well rename them _B and _E and take the arrays out of the function arguments.
*/
...
std::vector<double> sqrt_2Lplus1_pi              ( ell_max_total+1 );
std::vector<double> sqrt_2Lplus1_pi_over_L_Lplus1( ell_max_total+1 );
