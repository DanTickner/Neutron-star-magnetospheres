...
V33: Generalise double integral_trapezium_1D_theta() and double integral_trapezium_2D_theta() to accept any finite length/volume elements.
     Add Chebyshev polynomials (remapped to [r_min,r_max]), affine linear transformations Lambda and Lambda^-1, and calculation of Chebyshev series coefficients on an arbitrary interval (which is hardcoded as [r_min,r_max]).
*/
...
std::vector<double> calculate_chebyshev_series_coeffs( std::vector<double> f ){
	...
	for( int n=0; n<=n_max; n++ ){
		...
		//std::cout << integrand[10] << std::endl;
...
void VSH_decomposition_for_E(){
	...
	for( int i=0; i<n_points_r; i++ ){
		...
		for( int ell=0; ell<=ell_max; ell++ ){
			...
			E_VSH_coeffs[0][i][ell] = integral_trapezium_1D( VSH_integrand_r, finite_line_element_dt ) * sqrt_2Lplus1_pi[ell];
		...
		for( int ell=1; ell<=ell_max; ell++ ){
			...
			E_VSH_coeffs[1][i][ell] = integral_trapezium_1D( VSH_integrand_1, finite_line_element_dt ) * sqrt_2Lplus1_pi_over_L_Lplus1[ell];
			E_VSH_coeffs[2][i][ell] = integral_trapezium_1D( VSH_integrand_2, finite_line_element_dt ) * sqrt_2Lplus1_pi_over_L_Lplus1[ell];
...
void VSH_decomposition_for_B(){
	...
	for( int i=0; i<n_points_r; i++ ){
		...
		for( int ell=0; ell<=ell_max; ell++ ){
			...
			B_VSH_coeffs[0][i][ell] = integral_trapezium_1D( VSH_integrand_r, finite_line_element_dt ) * sqrt_2Lplus1_pi[ell];
		...
		for( int ell=1; ell<=ell_max; ell++ ){
			...
			B_VSH_coeffs[2][i][ell] = integral_trapezium_1D( VSH_integrand_2, finite_line_element_dt ) * sqrt_2Lplus1_pi_over_L_Lplus1[ell];
