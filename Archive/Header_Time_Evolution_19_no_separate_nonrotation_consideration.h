/*
Header_Time_Evolution_19_no_separate_nonrotation_consideration.h
...
V19: Remove separate ell_max for no rotation. However, keep separate BCs because Petri's BCs do not apply if there is no rotation.
...
void apply_inner_boundary_conditions( int T_index, int n ){
	...
	for( int j=0; j<n_points_t; j++ ){
		B_r_inner_before_BCs[j] = B_r[n][0][j];
		B_t_inner_before_BCs[j] = B_t[n][0][j];
		B_p_inner_before_BCs[j] = B_p[n][0][j];
		E_r_inner_before_BCs[j] = E_r[n][0][j];
		E_t_inner_before_BCs[j] = E_t[n][0][j];
		E_p_inner_before_BCs[j] = E_p[n][0][j];
	}
	...
	for( int j=0; j<n_points_t; j++ ){
		B_r_inner_after_BCs[j] = B_r[n][0][j];
		B_t_inner_after_BCs[j] = B_t[n][0][j];
		B_p_inner_after_BCs[j] = B_p[n][0][j];
		E_r_inner_after_BCs[j] = E_r[n][0][j];
		E_t_inner_after_BCs[j] = E_t[n][0][j];
		E_p_inner_after_BCs[j] = E_p[n][0][j];
	}
...
void apply_outer_boundary_conditions( int T_index, int n ){
	// Petri 2012, \S3.3.
	
	//--- Record values before BCs applied ---
	for( int j=0; j<n_points_t; j++ ){
		B_r_outer_before_BCs[j] = B_r[n][0][j];
		B_t_outer_before_BCs[j] = B_t[n][0][j];
		B_p_outer_before_BCs[j] = B_p[n][0][j];
		E_r_outer_before_BCs[j] = E_r[n][0][j];
		E_t_outer_before_BCs[j] = E_t[n][0][j];
		E_p_outer_before_BCs[j] = E_p[n][0][j];
	}
	...
	//--- Record values after BCs applied ---
	for( int j=0; j<n_points_t; j++ ){
		B_r_outer_after_BCs[j] = B_r[n][0][j];
		B_t_outer_after_BCs[j] = B_t[n][0][j];
		B_p_outer_after_BCs[j] = B_p[n][0][j];
		E_r_outer_after_BCs[j] = E_r[n][0][j];
		E_t_outer_after_BCs[j] = E_t[n][0][j];
		E_p_outer_after_BCs[j] = E_p[n][0][j];
	}
...
void output_to_csv_BCs( int T_index ){
