...
Remove the alpha and beta that I've been using up to this point, and replace by a current density vector J.
Rename div_E to E_div for consistency with E_curl and other functions.
FOUND TYPO: void calculate_time_derivatives_of_vector_components(): I forgot to multiply E_div by P0[j][L].
FOUND TYPO: void apply_force_free_conditions_within_force_free_region(): I had my else-statement in the wrong place so all gridpoints within FF region with FF conditions satisfied were set to -1 (denoting "outside FF region") instead of 0. Doesn't matter as it's only for plotting, but now fixed.
FOUND TYPO: void ramp_up_electric_fields_for_rotation(): Should be multiplying by 1/n Omega_max and not 1/n Omega. This is only a minor detail.
void ramp_up_electric_fields_for_rotation(): Rewrite to multiply by (Omega-Omega_prev), where Omega_prev is the value at the previous timestep, instead of 1/n * Omega(_max). Confirmed identical by run 20231208_c_run_to_T_equals_4_and_rewrite_rampup (values saved in log file) so can replace V2 by this new definition.
void output_to_csv_profiles(): Add optional parammeter bool condition_override = false which allows the user to force profile output at the current timestep even if the sampling conditions are not met.
