...
V15: Use ghost points for finite difference method. The alternative is to try again the Chebyshev decomposition in the radial direction.
     NOTE: V14 has been updated since V15 was created. Use at least V16 instead.
*/
...
std::vector<double> B_r_ghost_in  ( n_points_t );
std::vector<double> B_t_ghost_in  ( n_points_t );
std::vector<double> B_p_ghost_in  ( n_points_t );
std::vector<double> E_r_ghost_in  ( n_points_t );
std::vector<double> E_t_ghost_in  ( n_points_t );
std::vector<double> E_p_ghost_in  ( n_points_t );
std::vector<double> B_r_ghost_out ( n_points_t );
std::vector<double> B_t_ghost_out ( n_points_t );
std::vector<double> B_p_ghost_out ( n_points_t );
std::vector<double> E_r_ghost_out ( n_points_t );
std::vector<double> E_t_ghost_out ( n_points_t );
std::vector<double> E_p_ghost_out ( n_points_t );
...
void apply_initial_field_value_and_VSH_decomposition_for_B(){
	...
	for( int j=0; j<n_points_t; j++ ){
		B_r_ghost_in [j] = B_r_function( r_ghost_in , t[j] );
		B_t_ghost_in [j] = B_p_function( r_ghost_in , t[j] );
		B_p_ghost_in [j] = B_t_function( r_ghost_in , t[j] );
		B_r_ghost_out[j] = B_r_function( r_ghost_out, t[j] );
		B_t_ghost_out[j] = B_p_function( r_ghost_out, t[j] );
		B_p_ghost_out[j] = B_t_function( r_ghost_out, t[j] );
	}
}
