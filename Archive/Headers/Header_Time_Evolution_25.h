...
Simplify the ramp-up by splitting the value obtained by instantaneously applying rotation to a dipole, into n linear steps.
See OneNote Notes 20231115 and PhD/20231115 Expression for electric field ramp-up/Expression for electric field ramp-up.pdf.
n = T_index_Omega_ramp_stop - T_index_Omega_ramp_start + 1.
We are changing void ramp_up_electric_fields_for_rotation().
*/
...
std::vector< std::vector<bool  > > were_FF_conditions_applied ( n_points_r, std::vector<bool  > ( n_points_t ) );		// true/false to show the points at which the FF conditions were applied.
...
void calculate_dot_products(){
	...
	for( int i=0; i<n_points_r; i++ ){
		for( int j=0; j<n_points_t; j++ ){
			B_squared                [i][j] = pow( fields[0][i][j], 2 ) + pow( fields[2][i][j], 2 ) + pow( fields[3][i][j], 2 );
...
void apply_force_free_conditions_within_force_free_region(){
	...
	were_FF_conditions_applied = std::vector< std::vector<bool> > ( n_points_r, std::vector<bool> ( n_points_t ) );
			...
			for( int j=1; j<n_points_t-1; j++ ){
				if( r[i] * pow( st[j], -2 ) < 1.0 / Omega ){
					...
					if( ( E_dot_B[i][j] > E_dot_B_tol ) or ( B_squared_minus_E_squared[i][j] < 0 ) ){
						...
						were_FF_conditions_applied[i][j] = true;
