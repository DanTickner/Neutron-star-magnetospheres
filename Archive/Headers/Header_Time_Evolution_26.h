...
Calculations like the field ramp-up can influence the boundary values and they are done after the BCs are applied at each timestep.
We could change the loops to avoid the boundary values but this will lead to copy-and-pasting of loops with different start and endpoints, obfuscating the written code.
Instead, just do the full loops and reapply the BCs at the end of those functions.
Modify the BC function to make the recording of before-and-after values optional, and only do this in the main function call at the start of each timestep (not in the calls at the end of each function).
My for-loops within calculate_time_derivatives_of_vector_components were allowing the components to obtain nonzero time-derivatives even if they were prescribed by boundary conditions. These time-derivatives are now set back to zero at the end of the function.
Change array were_FF_conditions_applied from boolean to +1, -1, 0 to delineate the points which are not within the force-free region.
*/
...
std::vector< std::vector< std::vector<double> > > VSH_coeffs     ( 6, std::vector< std::vector<double> > ( n_points_r, std::vector<double> ( ell_max+1 ) ) );
std::vector< std::vector< std::vector<double> > > VSH_coeffs_dr  ( 6, std::vector< std::vector<double> > ( n_points_r, std::vector<double> ( ell_max+1 ) ) );
std::vector< std::vector< std::vector<double> > > VSH_coeffs_dr2 ( 6, std::vector< std::vector<double> > ( n_points_r, std::vector<double> ( ell_max+1 ) ) );

std::vector< std::vector< std::vector<double> > > fields           ( 6, std::vector< std::vector<double> > ( n_points_r, std::vector<double> ( n_points_t ) ) );
std::vector< std::vector< std::vector<double> > > fields_minus1    ( 6, std::vector< std::vector<double> > ( n_points_r, std::vector<double> ( n_points_t ) ) );
std::vector< std::vector< std::vector<double> > > fields_minus2    ( 6, std::vector< std::vector<double> > ( n_points_r, std::vector<double> ( n_points_t ) ) );
std::vector< std::vector< std::vector<double> > > fields_minus3    ( 6, std::vector< std::vector<double> > ( n_points_r, std::vector<double> ( n_points_t ) ) );
std::vector< std::vector< std::vector<double> > > fields_dT        ( 6, std::vector< std::vector<double> > ( n_points_r, std::vector<double> ( n_points_t ) ) );
std::vector< std::vector< std::vector<double> > > fields_dT_minus1 ( 6, std::vector< std::vector<double> > ( n_points_r, std::vector<double> ( n_points_t ) ) );
std::vector< std::vector< std::vector<double> > > fields_dT_minus2 ( 6, std::vector< std::vector<double> > ( n_points_r, std::vector<double> ( n_points_t ) ) );
std::vector< std::vector< std::vector<double> > > fields_dT_minus3 ( 6, std::vector< std::vector<double> > ( n_points_r, std::vector<double> ( n_points_t ) ) );
std::vector< std::vector< std::vector<double> > > fields_curl      ( 6, std::vector< std::vector<double> > ( n_points_r, std::vector<double> ( n_points_t ) ) );
std::vector< std::vector< std::vector<double> > > fields_Cartesian ( 6, std::vector< std::vector<double> > ( n_points_r, std::vector<double> ( n_points_t ) ) );
std::vector< std::vector< std::vector<double> > > fields_dr        ( 6, std::vector< std::vector<double> > ( n_points_r, std::vector<double> ( n_points_t ) ) );
std::vector< std::vector< std::vector<double> > > fields_dr2       ( 6, std::vector< std::vector<double> > ( n_points_r, std::vector<double> ( n_points_t ) ) );
std::vector< std::vector< std::vector<double> > > fields_series    ( 6, std::vector< std::vector<double> > ( n_points_r, std::vector<double> ( n_points_t ) ) );	// Values of the VSH decomposition of the field components.
std::vector< std::vector< std::vector<double> > > abs_rel_dev      ( 6, std::vector< std::vector<double> > ( n_points_r, std::vector<double> ( n_points_t ) ) );	// Absolute relative deviation of the VSH decomposition from the true values.
...
std::vector<double> stdev          ( 6 );	// Standard devation of the VSH decomposition of each vector component.
std::vector<double> max_abs_rel_dev( 6 );	// Maximum absolute relative deviation between the VSH decomposition and the exact value of each vector component.

double total_magnetic_energy = 0;


std::vector<int> nans ( 6 );	// Count the number of "nan" or "inf" values of each field component at a given timestep.
int nans_tot = 0;
...
void calculate_associated_legendre_functions(){
	...
	for( int j=0; j<n_points_t; j++ ){
		...
		for( int ell=2; ell<=ell_max; ell++ ){
			//P0[j][ell] = ( ( 2.0*ell-1.0 ) * x * P0[j][ell-1] - ( ell-1.0 ) * P0[j][ell-2] ) / ( (double)ell );
			P0[j][ell] = ( ( 2.0*ell+1.0 ) * x * P0[j][ell-1] - ell * P0[j][ell-2] ) / ( (double) ell+1.0 );
		}
		...
		for( int ell=3; ell<=ell_max; ell++ ){
			...
			P1[j][ell] = ( ( 2.0*ell+1.0 ) * x * P1[j][ell-1] - ( ell+1.0 ) * P1[j][ell-2] ) / ( (double) ell );
		}
...
void apply_inner_boundary_conditions( bool record_before_and_after = false ){
	...
	if( record_before_and_after ){
		for( int j=0; j<n_points_t; j++ ){
			for( int f=0; f<6; f++ ){
				fields_inner_before_BCs[f][j] = fields[f][0][j];
			}
		}
	}
	...
	//--- Apply inner BCs ---
	for( int j=0; j<n_points_t; j++ ){
		fields[0][0][j] = B_r_function( r_min, t[j] );
		fields[4][0][j] = B_r_function( r_min, t[j] ) * - Omega * r_min * st[j];
		fields[5][0][j] = 0;
	}
	...
	if( record_before_and_after ){
		for( int j=0; j<n_points_t; j++ ){
			for( int f=0; f<6; f++ ){
				fields_inner_after_BCs[f][j] = fields[f][0][j];
			}
		}
	}
...
void apply_outer_boundary_conditions( bool record_before_and_after = false ){
	...
	if( record_before_and_after ){
		for( int j=0; j<n_points_t; j++ ){
			for( int f=0; f<6; f++ ){
				fields_outer_before_BCs[f][j] = fields[f].back()[j];
			}
		}
	}
	...
	if( T_index < T_index_Omega_ramp_start ){
		for( int j=0; j<n_points_t; j++ ){
			fields[1].back()[j] = B_t_function( r_max, t[j] );
			fields[2].back()[j] = B_p_function( r_max, t[j] );
			fields[4].back()[j] = 0;
			fields[5].back()[j] = 0;
		}
	}
		...
		for( int j=0; j<n_points_t; j++ ){
			fields[1].back()[j] = 0.5 * ( fields[1].back()[j] - fields[5].back()[j] );
			fields[2].back()[j] = 0.5 * ( fields[4].back()[j] + fields[2].back()[j] );
			fields[4].back()[j] =   fields[2].back()[j];
			fields[5].back()[j] = - fields[1].back()[j];
		}
	}
	...
	if( record_before_and_after ){
		for( int j=0; j<n_points_t; j++ ){
			for( int f=0; f<6; f++ ){
				fields_outer_after_BCs[f][j] = fields[f].back()[j];
			}
		}
	}
...
void VSH_decomposition_for_E(){
	...
	for( int i=0; i<n_points_r; i++ ){
		...
		for( int ell=0; ell<=ell_max; ell++ ){
			...
			for( int j=0; j<n_points_t; j++ ){
				VSH_integrand_r[j] = fields[3][i][j] * P0[j][ell];
			}
			
			VSH_coeffs[3][i][ell] = integral_trapezium_sintheta_dtheta( VSH_integrand_r ) * sqrt_2Lplus1_pi[ell];
		...
		for( int ell=1; ell<=ell_max; ell++ ){
			...
			for( int j=0; j<n_points_t; j++ ){
				VSH_integrand_1[j] = fields[4][i][j] * P1[j][ell];
				VSH_integrand_2[j] = fields[5][i][j] * P1[j][ell];
			}
			
			VSH_coeffs[4][i][ell] = integral_trapezium_sintheta_dtheta( VSH_integrand_1 ) * sqrt_2Lplus1_pi_over_L_Lplus1[ell];
			VSH_coeffs[5][i][ell] = integral_trapezium_sintheta_dtheta( VSH_integrand_2 ) * sqrt_2Lplus1_pi_over_L_Lplus1[ell];
...
void VSH_decomposition_for_B(){
	...
	for( int i=0; i<n_points_r; i++ ){
		...
		for( int ell=0; ell<=ell_max; ell++ ){
			...
			for( int j=0; j<n_points_t; j++ ){
				VSH_integrand_r[j] = fields[0][i][j] * P0[j][ell];
			}
			
			VSH_coeffs[0][i][ell] = integral_trapezium_sintheta_dtheta( VSH_integrand_r ) * sqrt_2Lplus1_pi[ell];
		...
		for( int ell=1; ell<=ell_max; ell++ ){
			...
			for( int j=0; j<n_points_t; j++ ){
				VSH_integrand_2[j] = fields[2][i][j] * P1[j][ell];
			}
			
			VSH_coeffs[2][i][ell] = integral_trapezium_sintheta_dtheta( VSH_integrand_2 ) * sqrt_2Lplus1_pi_over_L_Lplus1[ell];
...
void calculate_radial_derivatives_of_vector_components(){
	...
	fields_dr  = std::vector< std::vector< std::vector<double> > > ( 6, std::vector< std::vector<double> > ( n_points_r, std::vector<double> ( n_points_t ) ) );
	fields_dr2 = std::vector< std::vector< std::vector<double> > > ( 6, std::vector< std::vector<double> > ( n_points_r, std::vector<double> ( n_points_t ) ) );
	...
	for( int i=0; i<n_points_r; i++ ){
		for( int j=0; j<n_points_t; j++ ){
			...
			for( int ell=0; ell<=ell_max; ell++ ){
				fields_dr [0][i][j] += sqrt_2Lplus1_over_4pi[ell] * P0[j][ell] * VSH_coeffs_dr [0][i][ell];				
				fields_dr [3][i][j] += sqrt_2Lplus1_over_4pi[ell] * P0[j][ell] * VSH_coeffs_dr [3][i][ell];
				fields_dr2[0][i][j] += sqrt_2Lplus1_over_4pi[ell] * P0[j][ell] * VSH_coeffs_dr2[0][i][ell];
			}
			
			for( int ell=1; ell<=ell_max; ell++ ){
				fields_dr [1][i][j] += sqrt_2Lplus1_over_4pi[ell] * P1[j][ell] * VSH_coeffs_dr [1][i][ell];
				fields_dr [2][i][j] += sqrt_2Lplus1_over_4pi[ell] * P1[j][ell] * VSH_coeffs_dr [2][i][ell];
				fields_dr [4][i][j] += sqrt_2Lplus1_over_4pi[ell] * P1[j][ell] * VSH_coeffs_dr [4][i][ell];
				fields_dr [5][i][j] += sqrt_2Lplus1_over_4pi[ell] * P1[j][ell] * VSH_coeffs_dr [5][i][ell];
			}
...
void calculate_Cartesian_vector_components(){
	...
	for( int i=0; i<n_points_r; i++ ){
		for( int j=0; j<n_points_t; j++ ){
			...
			fields_Cartesian[0][i][j] = fields[0][i][j] * st[j] + fields[1][i][j] * ct[j];
			fields_Cartesian[1][i][j] = fields[2][i][j];									// Extra variable assignment not necessary, but use for now to avoid confusion.
			fields_Cartesian[2][i][j] = fields[0][i][j] * ct[j] - fields[1][i][j] * st[j];
			
			fields_Cartesian[3][i][j] = fields[3][i][j] * st[j] + fields[4][i][j] * ct[j];
			fields_Cartesian[4][i][j] = fields[5][i][j];									// Extra variable assignment not necessary, but use for now to avoid confusion.
			fields_Cartesian[5][i][j] = fields[3][i][j] * ct[j] - fields[4][i][j] * st[j];
...
void apply_initial_field_values_for_B(){
	...
	for( int i=0; i<n_points_r; i++ ){
		for( int j=0; j<n_points_t; j++ ){
			fields[0][i][j] = B_r_function( r[i], t[j] );
			fields[1][i][j] = B_t_function( r[i], t[j] );
			fields[2][i][j] = B_p_function( r[i], t[j] );
		}
	}
}
...
void calculate_dot_products(){
	...
	for( int i=0; i<n_points_r; i++ ){
		for( int j=0; j<n_points_t; j++ ){
			B_squared                [i][j] = pow( fields[0][i][j], 2 ) + pow( fields[1][i][j], 2 ) + pow( fields[2][i][j], 2 );
			E_squared                [i][j] = pow( fields[3][i][j], 2 ) + pow( fields[4][i][j], 2 ) + pow( fields[5][i][j], 2 );
			B_squared_minus_E_squared[i][j] = B_squared[i][j] - E_squared[i][j];
			E_dot_B                  [i][j] = fields[3][i][j]*fields[0][i][j] + fields[4][i][j]*fields[1][i][j] + fields[5][i][j]*fields[2][i][j];
		}
	}
...
void ramp_up_electric_fields_for_rotation(){
	if( ( T_index >= T_index_Omega_ramp_start ) and ( T_index <= T_index_Omega_ramp_stop ) ){
		...
		for( int i=0; i<n_points_r; i++ ){
			for( int j=0; j<n_points_t; j++ ){
				fields[3][i][j] += dOmega_by_dT * delta_T * r[i] * st[j] *  fields[1][i][j];
				fields[4][i][j] += dOmega_by_dT * delta_T * r[i] * st[j] * -fields[0][i][j];
			}
		}
		*/
		
		//--- V2 (Header_Time_Evolution_25.h and OneNote Notes 20231115 and PhD/20231115 Expression for electric field ramp-up/Expression for electric field ramp-up.pdf) ---
		
		int n_for_linear_ramp = T_index_Omega_ramp_stop - T_index_Omega_ramp_start + 1;
		for( int i=0; i<n_points_r; i++ ){
			for( int j=0; j<n_points_t; j++ ){
				fields[3][i][j] += (double) (1.0/n_for_linear_ramp) * Omega * pow( r[i], -2 ) * pow( st[j], 2 );
				fields[4][i][j] += (double) (1.0/n_for_linear_ramp) * Omega * pow( r[i], -2 ) * -2.0 * st[j] * cos(t[j]);
			}
		}
			...
			for( int j=1; j<n_points_t-1; j++ ){
				if( r[i] * pow( st[j], -2 ) < 1.0 / Omega ){
					...
					if( ( E_dot_B[i][j] > E_dot_B_tol ) or ( B_squared_minus_E_squared[i][j] < 0 ) ){
						...
						double E_prime_r = fields[3][i][j] - E_dot_B_over_Bsq * fields[0][i][j];
						double E_prime_t = fields[4][i][j] - E_dot_B_over_Bsq * fields[1][i][j];
						double E_prime_p = fields[5][i][j] - E_dot_B_over_Bsq * fields[2][i][j];
						double sqrt_Bsq_over_Eprimesq = sqrt( B_squared[i][j] / ( E_prime_r*E_prime_r + E_prime_t*E_prime_t + E_prime_p*E_prime_p ) );
						
						fields[3][i][j] = E_prime_r * sqrt_Bsq_over_Eprimesq;
						fields[4][i][j] = E_prime_t * sqrt_Bsq_over_Eprimesq;
						fields[5][i][j] = E_prime_p * sqrt_Bsq_over_Eprimesq;
...
void calculate_time_derivatives_of_vector_components(){
	...
	fields_curl = std::vector< std::vector< std::vector<double> > > ( 6, std::vector< std::vector<double> > ( n_points_r, std::vector<double> ( n_points_t ) ) );
	div_E = std::vector< std::vector<double> > ( n_points_r, std::vector<double> ( n_points_t ) );
	...
	for( int i=0; i<n_points_r; i++ ){
		for( int j=0; j<n_points_t; j++ ){
			...
			//--- Spatial derivatives ---
			//- L=0 -
			div_E[i][j] += sqrt_2Lplus1_over_4pi[0] * ( VSH_coeffs_dr[3][i][0] + 2.0*pow(r[i],-1) * VSH_coeffs[3][i][0] );
			
			//- L>0 -
			for( int L=1; L<=ell_max; L++ ){
				
				div_E[i][j] += sqrt_2Lplus1_over_4pi[L] * ( VSH_coeffs_dr[3][i][L] + 2.0*pow(r[i],-1) * VSH_coeffs[3][i][L] - L*(L+1.0)*pow(r[i],-1) * VSH_coeffs[4][i][L] );
				
				fields_curl[0][i][j] += sqrt_2Lplus1_over_4pi[L] * P0[j][L] * -L*(L+1.0) * VSH_coeffs[2][i][L] / r[i];
				fields_curl[1][i][j] += sqrt_2Lplus1_over_4pi[L] * P1[j][L] * ( -VSH_coeffs_dr[2][i][L] - VSH_coeffs[2][i][L] / r[i] );
				fields_curl[2][i][j] += sqrt_2Lplus1_over_4pi[L] * P1[j][L] * ( r[i] * VSH_coeffs_dr2[0][i][L] + 4.0 * VSH_coeffs_dr[0][i][L] - (L+2.0)*(L-1.0) * VSH_coeffs[0][i][L] / r[i] ) / ( (double) L*(L+1.0) );
				
				fields_curl[3][i][j] += sqrt_2Lplus1_over_4pi[L] * P0[j][L] * -L*(L+1.0) * VSH_coeffs[5][i][L] / r[i];
				fields_curl[4][i][j] += sqrt_2Lplus1_over_4pi[L] * P1[j][L] * ( -VSH_coeffs_dr[5][i][L] - VSH_coeffs[5][i][L] / r[i] );
				fields_curl[5][i][j] += sqrt_2Lplus1_over_4pi[L] * P1[j][L] * ( VSH_coeffs_dr[4][i][L] + ( - VSH_coeffs[3][i][L] + VSH_coeffs[4][i][L] ) / r[i] );
				
			}
			
			//--- alpha and beta ---
			double B_dot_curl_B = fields[0][i][j]*fields_curl[0][i][j] + fields[1][i][j]*fields_curl[1][i][j] + fields[2][i][j]*fields_curl[2][i][j];
			double E_dot_curl_E = fields[3][i][j]*fields_curl[3][i][j] + fields[4][i][j]*fields_curl[4][i][j] + fields[5][i][j]*fields_curl[5][i][j];
			
			alpha[i][j] = ( E_dot_curl_E - B_dot_curl_B ) / B_squared[i][j];
			beta [i][j] = - div_E[i][j] / B_squared[i][j];
			
			//--- E cross B ---
			double E_cross_B_r = fields[4][i][j] * fields[2][i][j] - fields[5][i][j] * fields[1][i][j];
			double E_cross_B_t = fields[5][i][j] * fields[0][i][j] - fields[3][i][j] * fields[2][i][j];
			double E_cross_B_p = fields[3][i][j] * fields[1][i][j] - fields[4][i][j] * fields[0][i][j];
			
			//--- Time derivatives ---
			
			fields_dT[0][i][j] = - fields_curl[3][i][j];
			fields_dT[1][i][j] = - fields_curl[4][i][j];
			fields_dT[2][i][j] = - fields_curl[5][i][j];
			
			fields_dT[3][i][j] = fields_curl[0][i][j] + alpha[i][j] * fields[0][i][j] + beta[i][j] * E_cross_B_r;
			fields_dT[4][i][j] = fields_curl[1][i][j] + alpha[i][j] * fields[1][i][j] + beta[i][j] * E_cross_B_t;
			fields_dT[5][i][j] = fields_curl[2][i][j] + alpha[i][j] * fields[2][i][j] + beta[i][j] * E_cross_B_p;
			
			
			//--- Frictional term, applied to all components except B_r_dT ---
			if( use_outer_sponge_layer and ( r[i] >= r_sponge ) ){
				for( int f=1; f<6; f++ ){
					fields_dT[f][i][j] -= friction[i][j] * fields[f][i][j];
				}
			}
	...
	for( int j=0; j<n_points_t; j++ ){
		...
		fields_dT[4][0]    [j] = 0;
		fields_dT[5][0]    [j] = 0;
		fields_dT[1].back()[j] = 0;
		fields_dT[2].back()[j] = 0;
		fields_dT[4].back()[j] = 0;
		fields_dT[5].back()[j] = 0;
	}
}
...
void calculate_radial_derivatives_of_VSH_coeffs(){
	...
	for( int L=0; L<=ell_max; L++ ){
		...
		for( int f=0; f<6; f++ ){
			if( f != 1 ){
				VSH_coeffs_dr [f][0][L] = ( VSH_coeffs[f][1][L] - VSH_coeffs[f][0][L] ) / delta_r;
			}
		}
		
		VSH_coeffs_dr2[0][0][L] = ( VSH_coeffs[0][2][L] - 2.0*VSH_coeffs[0][1][L] + VSH_coeffs[0][0][L] ) / ( delta_r*delta_r );
		...
		for( int i=1; i<n_points_r-1; i++ ){
			...
			for( int f=0; f<6; f++ ){
				if( f != 1 ){
					VSH_coeffs_dr [f][i][L] = ( VSH_coeffs[f][i+1][L] - VSH_coeffs[f][i-1][L] ) / ( 2.0*delta_r );
				}
			}
					
			VSH_coeffs_dr2[0][i][L] = ( VSH_coeffs[0][i+1][L] - 2.0*VSH_coeffs[0][i][L] + VSH_coeffs[0][i-1][L] ) / ( delta_r*delta_r );
		}
		
		// Left derivatives at outer boundary.
		for( int f=0; f<6; f++ ){
			if( f != 1 ){
				VSH_coeffs_dr[f] .back()[L] = ( VSH_coeffs[f].back()[L] - VSH_coeffs[f][n_points_r-2][L] ) / delta_r;
			}
		}
		
		VSH_coeffs_dr2[0].back()[L] = ( VSH_coeffs[0].back()[L]  - 2.0*VSH_coeffs[0][n_points_r-2][L] + VSH_coeffs[0][n_points_r-3][L] ) / ( delta_r*delta_r );
...
void calculate_B_1_L_and_B_1_L_dr(){
	...
	for( int L=1; L<=ell_max; L++ ){
		for( int i=0; i<n_points_r; i++ ){
			VSH_coeffs   [1][i][L] = ( r[i] * VSH_coeffs_dr [0][i][L] + 2.0 * VSH_coeffs   [0][i][L] ) / ( (double) L*(L+1.0) );
			VSH_coeffs_dr[1][i][L] = ( r[i] * VSH_coeffs_dr2[0][i][L] + 3.0 * VSH_coeffs_dr[0][i][L] ) / ( (double) L*(L+1.0) );
		}
	}
...
void integrate_vectors_wrt_time_Adams_Bashforth_Order_1(){
	...
	for( int i=0; i<n_points_r; i++ ){
		for( int j=0; j<n_points_t; j++ ){
			...
			for( int f=0; f<6; f++ ){
				fields_minus1   [f][i][j] = fields   [f][i][j];
				fields_dT_minus1[f][i][j] = fields_dT[f][i][j];
			}
			
			//--- Calculate the values at the current timestep by Adams-Bashforth integration ---
			for( int f=0; f<6; f++ ){
				fields[f][i][j] = fields_minus1[f][i][j] + delta_T * fields_dT_minus1[f][i][j];
			}
...
void integrate_vectors_wrt_time_Adams_Bashforth_Order_2(){
	...
	for( int i=0; i<n_points_r; i++ ){
		for( int j=0; j<n_points_t; j++ ){
			...
			for( int f=0; f<6; f++ ){
				fields_minus2   [f][i][j] = fields_minus1   [f][i][j];
				fields_dT_minus2[f][i][j] = fields_dT_minus1[f][i][j];
			}
			...
			//--- Calculate the values at the current timestep by Adams-Bashforth integration ---
			for( int f=0; f<6; f++ ){
				fields[f][i][j] = fields_minus1[f][i][j] + delta_T * ( 1.5 * fields_dT_minus1[f][i][j] - 0.5 * fields_dT_minus2[f][i][j] );
			}
...
void integrate_vectors_wrt_time_Adams_Bashforth_Order_3(){
	...
	for( int i=0; i<n_points_r; i++ ){
		for( int j=0; j<n_points_t; j++ ){
			...
			for( int f=0; f<6; f++ ){
				fields_minus3   [f][i][j] = fields_minus2   [f][i][j];
				fields_dT_minus3[f][i][j] = fields_dT_minus2[f][i][j];
			}
			...
			//--- Calculate the values at the current timestep by Adams-Bashforth integration ---
			for( int f=0; f<6; f++ ){
				fields[f][i][j] = fields_minus1[f][i][j] + delta_T * ( 23.0 * fields_dT_minus1[f][i][j] - 16.0 * fields_dT_minus2[f][i][j] + 5.0 * fields_dT_minus3[f][i][j] ) / 12.0;
			}
...
void calculate_stdev(){
	...
	//--- Reset values to zero ---
	fields_series   = std::vector< std::vector< std::vector<double> > > ( 6, std::vector< std::vector<double> > ( n_points_r, std::vector<double> ( n_points_t ) ) );
	abs_rel_dev     = std::vector< std::vector< std::vector<double> > > ( 6, std::vector< std::vector<double> > ( n_points_r, std::vector<double> ( n_points_t ) ) );
	stdev           = std::vector<double> ( 6 );
	max_abs_rel_dev = std::vector<double> ( 6 );
	...
	for( int i=0; i<n_points_r; i++ ){
		for( int j=0; j<n_points_t; j++ ){
			...
			for( int ell=0; ell<=ell_max; ell++ ){
				fields_series[0][i][j] += sqrt_2Lplus1_over_4pi[ell] * P0[j][ell] * VSH_coeffs[0][i][ell];
				fields_series[3][i][j] += sqrt_2Lplus1_over_4pi[ell] * P0[j][ell] * VSH_coeffs[3][i][ell];
			}
			
			for( int ell=1; ell<=ell_max; ell++ ){
				fields_series[1][i][j] += sqrt_2Lplus1_over_4pi[ell] * P1[j][ell] * VSH_coeffs[1][i][ell];
				fields_series[2][i][j] += sqrt_2Lplus1_over_4pi[ell] * P1[j][ell] * VSH_coeffs[2][i][ell];
				fields_series[4][i][j] += sqrt_2Lplus1_over_4pi[ell] * P1[j][ell] * VSH_coeffs[4][i][ell];
				fields_series[5][i][j] += sqrt_2Lplus1_over_4pi[ell] * P1[j][ell] * VSH_coeffs[5][i][ell];
			}
			
			//--- Calculate stdev and build lists of square deviations along the way ---
			for( int f=0; f<6; f++ ){
				
				stdev[f] += pow( fields[f][i][j] - fields_series[f][i][j], 2 );
				
				if( fields[f][i][j] == 0 ){
					abs_rel_dev[f][i][j] = 0;
				}
				else {
					abs_rel_dev[f][i][j] = abs( 1.0 - fields_series[f][i][j] / fields[f][i][j] );
				}
				
				if( abs_rel_dev[f][i][j] > max_abs_rel_dev[f] ){
					max_abs_rel_dev[f] = abs_rel_dev[f][i][j];
				}
	...
	for( int f=0; f<6; f++ ){
		stdev[f] = sqrt( stdev[f] / ( (double) n_points_r * n_points_t ) );
	}
...
void calculate_magnetic_energy(){
	...
	for( int i=0; i<n_points_r; i++ ){
		for( int j=0; j<n_points_t; j++ ){
			magnetic_energy_density[i][j] = pow( fields[0][i][j], 2 ) + pow( fields[1][i][j], 2 ) + pow( fields[2][i][j], 2 );
		}
	}
...
double estimate_csv_profiles_filesize(){
	...
	int n_columns = 9 + 9 + 7 * 6;		// You are responsible for keeping this value updated.
	int n_rows = 0;
...
void output_headers_to_csv_profiles(){
	...
	output_file_profiles << ",B,E,alpha,beta,E_dot_B,B_squared_minus_E_squared,B_r_dr2,magnetic_energy_density,friction,were_FF_conditions_applied";
	
	output_file_profiles << ",B_r,B_t,B_p,E_r,E_t,E_p";
	output_file_profiles << ",B_x,B_y,B_z,E_x,E_y,E_z";
	...
	output_file_profiles << ",curl_B_r,curl_B_t,curl_B_p,curl_E_r,curl_E_t,curl_E_p";
		...
		for( int i=csv_profiles_write_i_min; i<n_points_r; i++ ){
			for( int j=0; j<n_points_t; j++ ){
				...
				if( condition_r and condition_t ){
					...
					std::vector<double> fields_dT_over_fields ( 6 );
					
					for( int f=0; f<6; f++ ){
						if( abs( fields[f][i][j] ) > tolerance_for_zero ){
							fields_dT_over_fields[f] = fields_dT[f][i][j] / fields[f][i][j];
						}
					}
					
					
					output_file_profiles <<std::setprecision(csv_precision)<< T_index <<","<< T <<","<< T*T_factor <<","<< i <<","<< j <<","<< r[i] <<","<< t[j] <<","<< x[i][j] <<","<< z[i][j]
																		   <<","<< sqrt(B_squared[i][j]) <<","<< sqrt(E_squared[i][j])
																		   <<","<< alpha[i][j] <<","<< beta[i][j]
																		   <<","<< E_dot_B[i][j] <<","<< B_squared_minus_E_squared[i][j]
																		   <<","<< fields_dr2[0][i][j]
																		   <<","<< magnetic_energy_density[i][j]
																		   <<","<< friction[i][j]
																		   <<","<< were_FF_conditions_applied[i][j];
					
					for( int f=0; f<6; f++ ){ output_file_profiles <<std::setprecision(csv_precision)<<","<< fields               [f][i][j]; }
					for( int f=0; f<6; f++ ){ output_file_profiles <<std::setprecision(csv_precision)<<","<< fields_Cartesian     [f][i][j]; }
					for( int f=0; f<6; f++ ){ output_file_profiles <<std::setprecision(csv_precision)<<","<< fields_dr            [f][i][j]; }
					for( int f=0; f<6; f++ ){ output_file_profiles <<std::setprecision(csv_precision)<<","<< abs_rel_dev          [f][i][j]; }
					for( int f=0; f<6; f++ ){ output_file_profiles <<std::setprecision(csv_precision)<<","<< fields_curl          [f][i][j]; }
					for( int f=0; f<6; f++ ){ output_file_profiles <<std::setprecision(csv_precision)<<","<< fields_dT            [f][i][j]; }
					for( int f=0; f<6; f++ ){ output_file_profiles <<std::setprecision(csv_precision)<<","<< fields_dT_over_fields[f]      ; }
...
void count_nans(){
	...
	nans = std::vector< int > ( 6 );
	
	for( int i=0; i<n_points_r; i++ ){
		for( int j=0; j<n_points_t; j++ ){
			for( int f=0; f<6; f++ ){
				if( ( isnan( fields[f][i][j] ) ) or ( isinf( fields[f][i][j] ) ) ){ nans[f] += 1; }
			}
		}
	}
	
	nans_tot = nans[0] + nans[1] + nans[2] + nans[3] + nans[4] + nans[5];
}
...
void output_to_csv_history(){
	...
	if( condition_T ){
		...
		for( int f=0; f<6; f++ ){ output_file_history <<std::setprecision(csv_precision)<<","<< stdev          [f]; }
		for( int f=0; f<6; f++ ){ output_file_history <<std::setprecision(csv_precision)<<","<< max_abs_rel_dev[f]; }
		for( int f=0; f<6; f++ ){ output_file_history <<std::setprecision(csv_precision)<<","<< nans           [f]; }
...
void output_headers_to_csv_VSH_coeffs(){
	...
	output_file_VSH_coeffs << "T_index,i,r,ell,B_r_L,B_1_L,B_2_L,E_r_L,E_1_L,E_2_L,B_r_L_dr,B_1_L_dr,B_2_L_dr,E_r_L_dr,E_1_L_dr,E_2_L_dr,B_r_L_dr2\n";
	...
	if( condition_T ){
		for( int i=0; i<n_points_r; i++ ){
			for( int ell=0; ell<=ell_max; ell++ ){
				...
				for( int f=0; f<6; f++ ){ output_file_VSH_coeffs <<std::setprecision(csv_precision)<<","<< VSH_coeffs   [f][i][ell]; }
				for( int f=0; f<6; f++ ){ output_file_VSH_coeffs <<std::setprecision(csv_precision)<<","<< VSH_coeffs_dr[f][i][ell]; }
				
				output_file_VSH_coeffs <<std::setprecision(csv_precision)<< ","<< VSH_coeffs_dr2[0][i][ell] << "\n";
...
void output_headers_to_csv_BCs(){
	...
	output_file_BCs << ",rel_diff_B_r_inner,rel_diff_B_t_inner,rel_diff_B_p_inner,rel_diff_E_r_inner,rel_diff_E_t_inner,rel_diff_E_p_inner";
	...
	output_file_BCs << ",rel_diff_B_r_outer,rel_diff_B_t_outer,rel_diff_B_p_outer,rel_diff_E_r_outer,rel_diff_E_t_outer,rel_diff_E_p_outer";
	output_file_BCs << "\n";
...
void output_to_csv_BCs(){
	...
	if( condition_T ){
		...
		for( int j=0; j<n_points_t; j++ ){
			...
			std::vector<double> rel_diff_fields_inner( 6 );
			std::vector<double> rel_diff_fields_outer( 6 );
			
			for( int f=0; f<6; f++ ){
				rel_diff_fields_inner[f] = 1.0 - fields_inner_after_BCs[f][j] / fields_inner_before_BCs[f][j];
				rel_diff_fields_outer[f] = 1.0 - fields_outer_after_BCs[f][j] / fields_outer_before_BCs[f][j];
			}
		
			output_file_BCs <<std::setprecision(csv_precision)<< T_index <<","<< T <<","<< T*T_factor <<","<< j <<","<< t[j];
			
			for( int f=0; f<6; f++ ){ output_file_BCs <<std::setprecision(csv_precision)<<","<< fields_inner_before_BCs[f][j]; }
			for( int f=0; f<6; f++ ){ output_file_BCs <<std::setprecision(csv_precision)<<","<< fields_inner_after_BCs [f][j]; }
			for( int f=0; f<6; f++ ){ output_file_BCs <<std::setprecision(csv_precision)<<","<< rel_diff_fields_inner  [f]   ; }
			for( int f=0; f<6; f++ ){ output_file_BCs <<std::setprecision(csv_precision)<<","<< fields_outer_before_BCs[f][j]; }
			for( int f=0; f<6; f++ ){ output_file_BCs <<std::setprecision(csv_precision)<<","<< fields_outer_after_BCs [f][j]; }
			for( int f=0; f<6; f++ ){ output_file_BCs <<std::setprecision(csv_precision)<<","<< rel_diff_fields_outer  [f]   ; }
...
void output_to_screen(){
	...
	if( ( T_index % cout_freq_T == 0 ) or ( T_index == n_timesteps ) ){
		...
		std::cout <<std::left<<std::setw(w)<< T_index <<std::left<<std::setw(w)<< T <<"|\t"
				  <<std::left<<std::setw(w)<< fields[0][cout_i][cout_j] <<std::left<<std::setw(w)<< fields[1][cout_i][cout_j] <<std::left<<std::setw(w)<< fields[2][cout_i][cout_j] <<"|\t"
				  <<std::left<<std::setw(w)<< fields[3][cout_i][cout_j] <<std::left<<std::setw(w)<< fields[4][cout_i][cout_j] <<std::left<<std::setw(w)<< fields[5][cout_i][cout_j] <<"|\t"
