...
Replace the 6-component vector fields[n][i][j] with E[n][i][j] and B[n][i][j] because it is difficult to know whether expressions are encoded correctly.
*/
...
void calculate_Cartesian_vector_components(){
	...
	for( int i=0; i<n_points_r; i++ ){
		for( int j=0; j<n_points_t; j++ ){
			...
			B_Cartesian[0][i][j] = E[0][i][j] * st[j] + E[1][i][j] * ct[j];
			B_Cartesian[1][i][j] = E[2][i][j];									// Extra variable assignment not necessary, but use for now to avoid confusion.
			B_Cartesian[2][i][j] = E[0][i][j] * ct[j] - E[1][i][j] * st[j];
...
void ramp_up_electric_fields_for_rotation(){
	if( ( T_index >= T_index_Omega_ramp_start ) and ( T_index <= T_index_Omega_ramp_stop ) ){
		...
		*/
		...
		for( int i=0; i<n_points_r; i++ ){
			for( int j=1; j<n_points_t-1; j++ ){
				E[0][i][j] += (double) (1.0/n_for_linear_ramp) * Omega * pow( r[i], -2 ) * pow( st[j], 2 );
				E[1][i][j] += (double) (1.0/n_for_linear_ramp) * Omega * pow( r[i], -2 ) * -2.0 * st[j] * cos(t[j]);
			}
		}
...
void calculate_time_derivatives_of_vector_components(){
	...
	for( int i=0; i<n_points_r; i++ ){
		for( int j=1; j<n_points_t-1; j++ ){
			...
			//--- Spatial derivatives ---
			//- L=0 -
			div_E[i][j] += sqrt_2Lplus1_over_4pi[0] * ( E_VSH_coeffs_dr[0][i][0] + 2.0*pow(r[i],-1) * E_VSH_coeffs[0][i][0] );
			
			//- L>0 -
			for( int L=1; L<=ell_max; L++ ){
				
				div_E[i][j] += sqrt_2Lplus1_over_4pi[L] * ( E_VSH_coeffs_dr[0][i][L] + 2.0*pow(r[i],-1) * E_VSH_coeffs[0][i][L] - L*(L+1.0)*pow(r[i],-1) * E_VSH_coeffs[1][i][L] );
			...
			//--- E cross B ---
			double E_cross_B_r = E[1][i][j] * B[2][i][j] - E[2][i][j] * B[1][i][j];
			double E_cross_B_t = E[2][i][j] * B[0][i][j] - E[0][i][j] * B[2][i][j];
			double E_cross_B_p = E[0][i][j] * B[1][i][j] - E[1][i][j] * B[0][i][j];
			
			//--- Time derivatives ---
			
			B_dT[0][i][j] = - E_curl[0][i][j];
			B_dT[1][i][j] = - E_curl[1][i][j];
			B_dT[2][i][j] = - E_curl[2][i][j];
			
			E_dT[0][i][j] = B_curl[0][i][j] + alpha[i][j] * B[0][i][j] + beta[i][j] * E_cross_B_r;
			E_dT[1][i][j] = B_curl[1][i][j] + alpha[i][j] * B[1][i][j] + beta[i][j] * E_cross_B_t;
			E_dT[2][i][j] = B_curl[2][i][j] + alpha[i][j] * B[2][i][j] + beta[i][j] * E_cross_B_p;
...
void calculate_stdev(){
	...
	for( int f=0; f<3; f++ ){
		B_stdev[f] = sqrt( B_stdev[f] / ( (double) n_points_r * n_points_t ) );
		E_stdev[f] = sqrt( E_stdev[f] / ( (double) n_points_r * n_points_t ) );
	}
...
void count_nans(){
	...
	for( int i=0; i<n_points_r; i++ ){
		for( int j=0; j<n_points_t; j++ ){
			for( int f=0; f<3; f++ ){
				if( ( isnan( B[f][i][j] ) ) or ( isinf( B[f][i][j] ) ) ){ B_nans[f] += 1; }
				if( ( isnan( E[f][i][j] ) ) or ( isinf( E[f][i][j] ) ) ){ E_nans[f] += 1; }
			}
		}
	}
