...
V12: Replace the trapezium integral functions over sin(theta) dtheta with some that include the sin(theta) in the infinitesimal. See OneNote 20230927.
...
void VSH_decomposition( std::vector< std::vector<double> >  A_r  , std::vector< std::vector<double> >  A_t  , std::vector< std::vector<double> >  A_p  ,
	...
	for( int i=0; i<n_points_r; i++ ){
		...
		for( int ell=0; ell<=ell_max; ell++ ){
			...
			for( int j=0; j<n_points_t; j++ ){
				VSH_integrand_r[j] = A_r[i][j] * P0[j][ell];
			}
		...
		for( int ell=1; ell<=ell_max; ell++ ){
			...
			for( int j=0; j<n_points_t; j++ ){
				VSH_integrand_1[j] = A_t[i][j] * P1[j][ell];
				VSH_integrand_2[j] = A_p[i][j] * P1[j][ell];
			}
...
void evaluate_VSH_series(){
	...
	std::cout << "Function void evaluate_VSH_series() being called." << std::endl;
	...
	for( int i=0; i<n_points_r; i++ ){
		for( int j=0; j<n_points_t; j++ ){
			...
			for( int ell=0; ell<=ell_max; ell++ ){
				B_r[i][j] += sqrt_2Lplus1_over_4pi[ell] * P0[j][ell] * B_r_L[i][ell];
				E_r[i][j] += sqrt_2Lplus1_over_4pi[ell] * P0[j][ell] * E_r_L[i][ell];
				
			}
			
			for( int ell=1; ell<=ell_max; ell++ ){
				B_t[i][j] += sqrt_2Lplus1_over_4pi[ell] * P1[j][ell] * B_1_L[i][ell];
				B_p[i][j] += sqrt_2Lplus1_over_4pi[ell] * P1[j][ell] * B_2_L[i][ell];
				E_t[i][j] += sqrt_2Lplus1_over_4pi[ell] * P1[j][ell] * E_1_L[i][ell];
				E_p[i][j] += sqrt_2Lplus1_over_4pi[ell] * P1[j][ell] * E_2_L[i][ell];
