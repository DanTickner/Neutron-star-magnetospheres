...
V04: The fields in fact *do* change at the inner boundaries, so we *do* need to calculate the radial finite difference derivatives there.
...
void apply_outer_boundary_conditions(){
	// Petri 2012, \S3.3.
	for( int j=0; j<n_points_t; j++ ){
		B_t.back()[j] = 10 * 0.5 * ( B_t.back()[j] - E_p.back()[j] );
		B_p.back()[j] = 10 * 0.5 * ( E_t.back()[j] + B_p.back()[j] );
...
void calculate_stdev(){
	...
	for( int i=0; i<n_points_r; i++ ){
		for( int j=0; j<n_points_t; j++ ){
			for( int ell=0; ell<=ell_max; ell++ ){
				...
				B_r_series[i][j] += sqrt_2Lplus1_over_4pi[i] * P0[j][ell] * B_r_L[i][ell];
				B_t_series[i][j] += sqrt_2Lplus1_over_4pi[i] * P1[j][ell] * B_1_L[i][ell];
				B_t_series[i][j] += sqrt_2Lplus1_over_4pi[i] * P1[j][ell] * B_2_L[i][ell];
				
				E_r_series[i][j] += sqrt_2Lplus1_over_4pi[i] * P0[j][ell] * E_r_L[i][ell];
				E_t_series[i][j] += sqrt_2Lplus1_over_4pi[i] * P1[j][ell] * E_1_L[i][ell];
				E_t_series[i][j] += sqrt_2Lplus1_over_4pi[i] * P1[j][ell] * E_2_L[i][ell];
	...
	std::cout << stdev_B_r <<"\t"<< stdev_B_t <<"\t"<< stdev_B_p <<"\t"<< stdev_E_r <<"\t"<< stdev_E_t <<"\t"<< stdev_E_p <<"\t"<< std::endl;
