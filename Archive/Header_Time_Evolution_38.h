...
V38: Rename n_points_r to n_r and n_points_t to n_t for brevity.
     In void calculate_stdev_of_VSH_decomposition(), turn the if-else with abs_rel_dev into just if, since the values are zero by default.
	 Remove extra std::setprecision(precision_csv) in output functions since only the first is needed and it hurts readability.
	 ...
	 Set more variables to constant.
	 In void calculate_time_and_rotation_values() , Omega[ T_index_rotation_ramp_start ] = dOmega_by_dT_index; was redundant.
*/
...
const double B_factor = mu_0/(4.0*pi) * m_star * pow( R_star, -3 );
...
std::vector< std::vector<double> > radial_derivatives_of_VSH_coeff_FD( std::vector< std::vector< std::vector<double> > > v_VSH_coeffs, int f ){
	// Calculate the radial derivative of the f-th component of a VSH series, evaluated at all radial gridpoints, by finite differencing.
	
	std::vector< std::vector<double> > v_VSH_coeffs_dr ( ell_max+1, std::vector<double> ( n_r ) );
	
	for( int L=0; L<=ell_max; L++ ){
		
		// Right derivatives at inner boundary.
		v_VSH_coeffs_dr[L][0] = ( v_VSH_coeffs[f][L][1] - v_VSH_coeffs[f][L][0] ) / delta_r;
		
		// Symmetric derivatives for all intermediate points.
		for( int i=1; i<n_r-1; i++ ){
			v_VSH_coeffs_dr[L][i] = ( v_VSH_coeffs[f][L][i+1] - v_VSH_coeffs[f][L][i-1] ) / ( 2.0*delta_r );
		}
		
		// Left derivatives at outer boundary.
		v_VSH_coeffs_dr[L].back() = ( v_VSH_coeffs[f][L].back() - v_VSH_coeffs[f][L][n_r-2] ) / delta_r;
		
	}
	
	return v_VSH_coeffs_dr;
}
...
std::vector< std::vector<double> > second_radial_derivatives_of_VSH_coeff_FD( std::vector< std::vector< std::vector<double> > > v_VSH_coeffs, int f ){
	// Calculate the second radial derivative of the f-th component of a VSH series, evaluated at all radial gridpoints, by finite differencing.
	
	std::vector< std::vector<double> > v_VSH_coeffs_dr2 ( ell_max+1, std::vector<double> ( n_r ) );
	
	for( int L=0; L<=ell_max; L++ ){
		
		// Right derivatives at inner boundary.
		v_VSH_coeffs_dr2[L][0] = ( v_VSH_coeffs[f][L][2] - 2.0*v_VSH_coeffs[f][L][1] + v_VSH_coeffs[f][L][0] ) / ( delta_r*delta_r );
		
		// Symmetric derivatives for all intermediate points.
		for( int i=1; i<n_r-1; i++ ){
			v_VSH_coeffs_dr2[L][i] = ( v_VSH_coeffs[f][L][i+1] - 2.0*v_VSH_coeffs[f][L][i] + v_VSH_coeffs[f][L][i-1] ) / ( delta_r*delta_r );
		}
		
		// Left derivatives at outer boundary.
		v_VSH_coeffs_dr2[L].back() = ( v_VSH_coeffs[f][L].back()  - 2.0*v_VSH_coeffs[f][L][n_r-2] + v_VSH_coeffs[f][L][n_r-3] ) / ( delta_r*delta_r );
		
	}
	
	return v_VSH_coeffs_dr2;
}




std::vector< std::vector<double> > radial_derivatives_of_VSH_coeff_Chebyshev( std::vector< std::vector< std::vector<double> > > v_VSH_coeffs, int f ){
	// Calculate the radial derivative of the f-th component of a VSH series, evaluated at all radial gridpoints, by its Chebyshev series.
	
	std::vector< std::vector<double> > v_VSH_coeffs_dr ( ell_max+1, std::vector<double> ( n_r ) );
	
	for( int L=0; L<=ell_max; L++ ){
			
		std::vector<double> chebyshev_coeffs = chebyshev_series_coeffs( v_VSH_coeffs[f][L] );
	
		for( int i=0; i<n_r; i++ ){
			v_VSH_coeffs_dr[L][i] = evaluate_chebyshev_series_first_derivative_at_gridpoint( chebyshev_coeffs, i );
		}
...
std::vector< std::vector<double> > divergence_from_VSH_coeffs( std::vector< std::vector< std::vector<double> > > &v_VSH_coeffs, std::vector< std::vector< std::vector<double> > > &v_VSH_coeffs_dr ){
	...
	for( int i=0; i<n_r; i++ ){
		for( int j=0; j<n_t; j++ ){
			...
			//--- L = 0 ---
			v_div[i][j] += sqrt_2Lplus1_over_4pi[0] * ( v_VSH_coeffs_dr[0][0][i] + 2.0*pow(r[i],-1) * v_VSH_coeffs[0][0][i] );
			
			//--- L > 0 ---
			for( int L=1; L<=ell_max; L++ ){
				v_div[i][j] += sqrt_2Lplus1_over_4pi[L] * P0[j][L] * ( v_VSH_coeffs_dr[0][L][i] + 2.0*pow(r[i],-1) * v_VSH_coeffs[0][L][i] - L*(L+1.0)*pow(r[i],-1) * v_VSH_coeffs[1][L][i] );
			}
