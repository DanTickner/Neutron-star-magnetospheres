...
V13: Include functionality for first-order Adams-Bashforth integration. This is really just the Euler method so no change to the output, but the framework is in place for higher-order Adams-Bashforth methods.
     void evaluate_VSH_series() is never used because we only make a VSH decomposition to obtain div(E), curl(B) and curl(E). Delete this function - it can be recovered from archived header files if needed again.
*/
...
std::vector< std::vector< std::vector<double> > > B_r ( 2, std::vector< std::vector<double> > ( n_points_r, std::vector<double> ( n_points_t ) ) );	// Store the values at the previous few timesteps for use with Adams-Bashforth integration. Don't use new variables for each timestep because otherwise we have to pass the right arrays as arguments to all the functions and this becomes extremely slow. This way, we only need to pass the index n.
std::vector< std::vector< std::vector<double> > > B_t ( 2, std::vector< std::vector<double> > ( n_points_r, std::vector<double> ( n_points_t ) ) );
std::vector< std::vector< std::vector<double> > > B_p ( 2, std::vector< std::vector<double> > ( n_points_r, std::vector<double> ( n_points_t ) ) );
std::vector< std::vector< std::vector<double> > > E_r ( 2, std::vector< std::vector<double> > ( n_points_r, std::vector<double> ( n_points_t ) ) );
std::vector< std::vector< std::vector<double> > > E_t ( 2, std::vector< std::vector<double> > ( n_points_r, std::vector<double> ( n_points_t ) ) );
std::vector< std::vector< std::vector<double> > > E_p ( 2, std::vector< std::vector<double> > ( n_points_r, std::vector<double> ( n_points_t ) ) );

std::vector< std::vector< std::vector<double> > > B_r_dT ( 2, std::vector< std::vector<double> > ( n_points_r, std::vector<double> ( n_points_t ) ) );
std::vector< std::vector< std::vector<double> > > B_t_dT ( 2, std::vector< std::vector<double> > ( n_points_r, std::vector<double> ( n_points_t ) ) );
std::vector< std::vector< std::vector<double> > > B_p_dT ( 2, std::vector< std::vector<double> > ( n_points_r, std::vector<double> ( n_points_t ) ) );
std::vector< std::vector< std::vector<double> > > E_r_dT ( 2, std::vector< std::vector<double> > ( n_points_r, std::vector<double> ( n_points_t ) ) );
std::vector< std::vector< std::vector<double> > > E_t_dT ( 2, std::vector< std::vector<double> > ( n_points_r, std::vector<double> ( n_points_t ) ) );
std::vector< std::vector< std::vector<double> > > E_p_dT ( 2, std::vector< std::vector<double> > ( n_points_r, std::vector<double> ( n_points_t ) ) );
...
void calculate_time_derivatives( int T_index, int n ){
	...
	for( int i=0; i<n_points_r; i++ ){
		for( int j=0; j<n_points_t; j++ ){
			...
			//--- Time derivatives ---
			int n_for_dT = std::min( abs( n-1 ), 0 );
...
/*void integrate_vectors_wrt_time_Adams_Bashforth_Order_2(){
	...
	for( int i=0; i<n_points_r; i++ ){
		for( int j=0; j<n_points_t; j++ ){
			B_r_nplus2[i][j] += B_r_dT_n[i][j] * delta_T;
			B_t_nplus2[i][j] += B_t_dT_n[i][j] * delta_T;
			B_p_nplus2[i][j] += B_p_dT_n[i][j] * delta_T;
			E_r_nplus2[i][j] += E_r_dT_n[i][j] * delta_T;
			E_t_nplus2[i][j] += E_t_dT_n[i][j] * delta_T;
			E_p_nplus2[i][j] += E_p_dT_n[i][j] * delta_T;
		}
	}
}*/
...
void apply_inner_boundary_conditions( int T_index, int n ){
	// Petri 2012, \S3.3.
	for( int j=0; j<n_points_t; j++ ){
		B_r[n][0][j] = B_r_function( r[0], t[j] );
		E_t[n][0][j] = - Omega * r[0] * st[j] * B_r_function( r[0], t[j] );
		E_p[n][0][j] = 0;
	}
}
...
void output_headers_to_csv_history(){
	...
	output_file_history << ",Omega,star_rotation_angle";
