...
V11: Expression for dE/dT during the ramping-up phase of rotation depends on the chosen magnetic field, so has been relegated to the header file.
	 ...
	 Re-added outer sponger layer (Parfrey, 2012, PhD thesis, S3.10 and Header_Time_Evolution_08.h). Activated via bool use_outer_sponge_layer in Initial_conditions_02.h.
	 Remove variable R_LC and replace by 1.0 / Omega because Omega is a variable. Similarly, calculate the number of gridpoints within the FF region at each timestep. To make this easier, just put void calculate_n_gridpoints_within_force_free_region() inside void apply_force_free_conditions_within_force_free_region().
*/
...
void VSH_decomposition( std::vector< std::vector<double> >  A_r  , std::vector< std::vector<double> >  A_t  , std::vector< std::vector<double> >  A_p  ,
	...
	for( int i=0; i<n_points_r; i++ ){
		...
		for( int ell=0; ell<=ell_max; ell++ ){
			...
			for( int j=0; j<n_points_t; j++ ){
				VSH_integrand_r[j] = A_r[i][j] * P0[j][ell] * st[j];
			}
			
			A_r_L[i][ell] = integral_trapezium( VSH_integrand_r, t ) * sqrt( (2.0*ell+1.0)*pi );
		...
		for( int ell=1; ell<=ell_max; ell++ ){
			...
			for( int j=0; j<n_points_t; j++ ){
				VSH_integrand_1[j] = A_t[i][j] * P1[j][ell] * st[j];
				VSH_integrand_2[j] = A_p[i][j] * P1[j][ell] * st[j];
			}
			
			A_1_L[i][ell] = integral_trapezium( VSH_integrand_1, t ) * sqrt( (2.0*ell+1.0)*pi ) / ( (double)ell*(ell+1.0) );
			A_2_L[i][ell] = integral_trapezium( VSH_integrand_2, t ) * sqrt( (2.0*ell+1.0)*pi ) / ( (double)ell*(ell+1.0) );
	...
	for( int i=0; i<n_points_r; i++ ){
		for( int j=0; j<n_points_t; j++ ){
			...
			for( int ell=1; ell<=ell_max; ell++ ){
				...
				B_p[i][j] += sqrt_2Lplus1_over_4pi[i] * P1[j][ell] * B_2_L[i][ell];
				E_t[i][j] += sqrt_2Lplus1_over_4pi[i] * P1[j][ell] * E_1_L[i][ell];
				E_p[i][j] += sqrt_2Lplus1_over_4pi[i] * P1[j][ell] * E_2_L[i][ell];
...
void calculate_time_derivatives( int T_index ){
	for( int i=0; i<n_points_r; i++ ){
		for( int j=0; j<n_points_t; j++ ){
			...
			B_t_dT[i][j] = - curl_E_t[i][j] - friction[i][j] * B_t[i][j];
			B_p_dT[i][j] = - curl_E_p[i][j] - friction[i][j] * B_p[i][j];
			
			E_r_dT[i][j] = curl_B_r[i][j] + alpha[i][j] * B_r[i][j] + beta[i][j] * E_cross_B_r - friction[i][j] * E_r[i][j];
			E_t_dT[i][j] = curl_B_t[i][j] + alpha[i][j] * B_t[i][j] + beta[i][j] * E_cross_B_t - friction[i][j] * E_t[i][j];
			E_p_dT[i][j] = curl_B_p[i][j] + alpha[i][j] * B_p[i][j] + beta[i][j] * E_cross_B_p - friction[i][j] * E_p[i][j];
...
void apply_inner_boundary_conditions( int T_index ){
	// Petri 2012, \S3.3.
	for( int j=0; j<n_points_t; j++ ){
		B_r[0][j] = B_r_function( r[0], t[j] );
		E_t[0][j] = - Omega * r[0] * st[j] * B_r_function( r[0], t[j] );
		E_p[0][j] = 0;
	}
}
...
void calculate_stdev(){
	...
	for( int i=0; i<n_points_r; i++ ){
		for( int j=0; j<n_points_t; j++ ){
			...
			abs_dev_B_r[i][j] = abs( B_r[i][j] - B_r_series[i][j] );
			abs_dev_B_t[i][j] = abs( B_t[i][j] - B_t_series[i][j] );
			abs_dev_B_p[i][j] = abs( B_p[i][j] - B_p_series[i][j] );
			abs_dev_E_r[i][j] = abs( E_r[i][j] - E_r_series[i][j] );
			abs_dev_E_t[i][j] = abs( E_t[i][j] - E_t_series[i][j] );
			abs_dev_E_p[i][j] = abs( E_p[i][j] - E_p_series[i][j] );
