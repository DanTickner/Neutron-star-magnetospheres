...
V44: Add function void ramp_up_electric_fields_for_twist() which allows for a custom Omega to be added at each timestep, used for slowly imparting a magnetospheric twist.
*/
	...
	if( ( T_index >= T_index_twist_ramp_start ) and ( T_index <= T_index_twist_ramp_stop ) ){
		...
		for( int i=0; i<n_r; i++ ){
			for( int j=0; j<n_t; j++ ){
				...
				double factor = twist_final_Omega * r[i] * st[j] / (double) n_steps_twist;
