...
V10: Support linear increase in rotation rate between T_index_Omega_ramp_start and T_index_Omega_ramp_stop.
*/
	...
	for( int i=0; i<n_points_r; i++ ){
		for( int j=0; j<n_points_t; j++ ){
			...
			if( ( T_index >= T_index_Omega_ramp_start ) and ( T_index <= T_index_Omega_ramp_stop ) ){
				// The added terms here are ONLY valid for a dipole.
				if( ( i == cout_i ) and ( j == cout_j ) ){ std::cout << "Ramping up Omega\tt = " << T_index << std::endl; }
				E_r_dT[i][j] += dOmega_by_dT * pow( st[j] / r[i], 2 );
				E_t_dT[i][j] += dOmega_by_dT * -2.0 * st[j] * ct[j] * pow( r[i], -2 );
			}
