...
g++ Calculate_Finite_Difference_Coefficients_General_First_Derivative.cpp -o Calculate_Finite_Difference_Coefficients_General_First_Derivative --std=c++11
Calculate_Finite_Difference_Coefficients_General_First_Derivative
...
int main(){
	...
	int N_B    = 1;	// number of gridpoints backward we look.
	int N_F    = 7;	// number of gridpoints forward  we look.
	int N_eqns = N_B + N_F - 1;	// The number of simultaneous equations.
	
	double a_N_F = 15;	// change this result to guarantee integers.
	
	std::string output_filename_test_base = "20240621_Finite_difference_coefficients_general_first_derivative";	// Don't add ".csv" as the code will append _r, _t, etc to the filename for each csv.
	...
	std::cout << "Backward points N_B:\t" << N_B         << std::endl;
	std::cout << "Forward  points N_F:\t" << N_F << "\n" << std::endl;
	...
	std::cout << "\nSetting a_{" << N_F << "} = " << a_N_F << ". Change this value and run the code if this doesn't match expectations." << std::endl;
