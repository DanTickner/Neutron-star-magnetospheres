...
g++ Test_VSH_series_axisymmetric_zero_divergence_Chebyshev.cpp -o Test_VSH_series_axisymmetric_zero_divergence_Chebyshev --std=c++11
Test_VSH_series_axisymmetric_zero_divergence_Chebyshev
...
#include "../Header_Time_Evolution_34.h"
...
int main(){
	...
	std::string output_filename_test_base = "20240308_Test_VSH_series_axisymmetric_zero_divergence_Chebyshev";
	...
	for( int i=0; i<n_points_r; i++ ){
		...
		for( int k=0; k<3; k++ ){
			...
			//stdev_values_r[k][i] = 0;//stdev_of_array( B[k][i], B_from_VSH_coeffs[k][i] );
	...
	for( int j=0; j<n_points_t; j++ ){
		...
		for( int k=0; k<3; k++ ){
			...
			// Calculate this stdev manually due to array indexing.
			for( int i=0; i<n_points_r; i++ ){
