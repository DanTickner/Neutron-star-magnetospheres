...
#----- Define variables -----
csv_filename = "../CSV/20240405_Test_radial_derivatives_FD_function_values.csv"

fig_filename_base = "../Figures/20240405/20240405_Test_radial_derivatives_FD"
...
f_dr_abs_rel_err_list  = np.zeros( len( i_list ) )
f_dr2_abs_rel_err_list = np.zeros( len( i_list ) )

for i in range( len( i_list ) ):
	
	if f_dr_exact_list[i] != 0:
		f_dr_abs_rel_err_list[i]  = abs( 1.0 - f_dr_calc_list [i] / f_dr_exact_list [i] )
	
	if f_dr2_exact_list[i] != 0 :
		f_dr2_abs_rel_err_list[i] = abs( 1.0 - f_dr2_calc_list[i] / f_dr2_exact_list[i] )
