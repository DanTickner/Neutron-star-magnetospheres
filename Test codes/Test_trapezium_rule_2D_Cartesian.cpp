...
g++ Test_trapezium_rule_2D_Cartesian.cpp -o Test_trapezium_rule_2D_Cartesian --std=c++11
Test_trapezium_rule_2D_Cartesian
...
int main(){
	...
	//----- Define variables -----
	std::string output_filename_test_log = "../Logs/20240227_Test_trapezium_rule_2D_Cartesian.txt";
	...
	std::cout            << "i\tj\tu1[i]\tu2[j]\tf[i][j]\tdA[i][j]\tDelta x*Delta y" << std::endl;
	output_file_test_log << "i\tj\tu1[i]\tu2[j]\tf[i][j]\tdA[i][j]\tDelta x*Delta y" << "\n";
	
	for( int i=0; i<N_values_u1; i++ ){
		for( int j=0; j<N_values_u2; j++ ){
			std::cout            << i <<"\t"<< j <<"\t"<< u1[i] <<"\t"<< u2[j] <<"\t"<< f[i][j] <<"\t"<< dA[i][j] <<"\t"<< delta_u1 * delta_u2 << std::endl;
			output_file_test_log << i <<"\t"<< j <<"\t"<< u1[i] <<"\t"<< u2[j] <<"\t"<< f[i][j] <<"\t"<< dA[i][j] <<"\t"<< delta_u1 * delta_u2 << "\n";
		}
	}
