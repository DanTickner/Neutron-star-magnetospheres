...
Test the calculation of the VSH series of an axisymmetric spherical polar vector B.
Two possible expressions to test.
...
We have already shown that calculating radial derivatives by a Chebyshev series is less accurate than finite differencing in our code.
For a previous version of this code that indeed allowed using a Chebysev series, see Archive/Test_VSH_series_axisymmetric_20240402.cpp.

Include the expressions for B in this code, not Header_Initial_conditions_xx.h,
so that we can use more exotic functions for testing and store the exact VSH coefficients in the same place,
without worrying about desynchronisation between the chosen functions and the exact VSH coefficients.

Output CSV files with vector values, the VSH series coefficients as a function of r, and standard deviations as a function of each coordinate.
*/
...
int main(){
	...
	int test_type = 2;	// 1 = General; 2 = Divergenceless with finite differencing.
	
	std::string output_filename_test_base = "20240502_Test_VSH_series_axisymmetric";
	...
	std::vector< std::vector< std::vector<double> > > VSH_coeffs_exact      ( 3, std::vector< std::vector<double> > ( ell_max+1, std::vector<double> ( n_r ) ) );	// Known VSH coefficients, to be input manually.
	
	std::vector< std::vector< double> > stdev_coeffs   ( 3, std::vector<double> ( ell_max+1 ) );
	std::vector< std::vector< double> > stdev_values_r ( 3, std::vector<double> ( n_r ) );	// Stdev of recalculated vector as a function of r.
	std::vector< std::vector< double> > stdev_values_t ( 3, std::vector<double> ( n_t ) );	// Stdev of recalculated vector as a function of theta.
	...
	int n_total_limit_for_vector_output_to_csv = 3e5;	// Failsafe to avoid accidentally making huge files when the above is left as true.
	...
	for( int i=0; i<n_r; i++ ){
		for( int ell=0; ell<=ell_max; ell++ ){
			VSH_coeffs_exact[0][ell][i] = B_VSH_coefficient_exact_r_test( r[i], ell );
			VSH_coeffs_exact[1][ell][i] = B_VSH_coefficient_exact_1_test( r[i], ell );
			VSH_coeffs_exact[2][ell][i] = B_VSH_coefficient_exact_2_test( r[i], ell );
		}
	}
	...
	for( int k=0; k<3; k++ ){
		for( int ell=0; ell<=ell_max; ell++ ){
			...
			stdev_coeffs[k][ell] = stdev_of_array_1d( B_VSH_coeffs[k][ell], VSH_coeffs_exact[k][ell] );
	...
	if( n_r * n_t > n_total_limit_for_vector_output_to_csv ){
	...
	for( int i=0; i<n_r; i++ ){
		...
		for( int k=0; k<3; k++ ){
			...
			stdev_values_r[k][i] = sqrt( stdev_values_r[k][i] / ( (double) n_t ) );
	...
	for( int j=0; j<n_t; j++ ){
		...
		for( int k=0; k<3; k++ ){
			...
			stdev_values_t[k][j] = sqrt( stdev_values_t[k][j] / ( (double) n_r ) );
