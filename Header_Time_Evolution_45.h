...
V45: Turn twist_final_Omega from double to 2D vector. Add function void setup_twist_final_Omega() to populate the final values instead of doing it every timestep in void ramp_up_electric_fields_for_twist().
     Changed logic for apply_force_free_conditions_within_force_free_region() as the array were_FF_conditions_applied was not being populated properly. 
*/
...
std::string output_filename_time_values = output_folder + "CSV/"   + output_filename + "_1_time_values.csv";
std::string output_filename_gridpoints  = output_folder + "CSV/"   + output_filename + "_2_gridpoints.csv";
std::string output_filename_profiles    = output_folder + "CSV/"   + output_filename + "_4_profiles.csv";
std::string output_filename_history     = output_folder + "CSV/"   + output_filename + "_5_history.csv";
std::string output_filename_VSH_coeffs  = output_folder + "CSV/"   + output_filename + "_6_VSH_coeffs.csv";
std::string output_filename_BCs         = output_folder + "CSV/"   + output_filename + "_7_BCs.csv";
std::string output_filename_log         = output_folder + "Logs/"  + output_filename + ".txt";
...
std::vector< std::vector<double> > twist_final_Omega_values      ( n_r, std::vector<double> ( n_t ) );		// Store the smoothed values of Omega for magnetospheric twists. These are gradually added to the electric field over time.
	...
	if( ( T_index >= T_index_rotation_ramp_start ) and ( T_index <= T_index_rotation_ramp_stop ) ){
		...
		for( int i=0; i<n_r; i++ ){
			for( int j=0; j<n_t; j++ ){
				...
				//--- Version 1: use hardcoded expression for rotating dipole ---
				//E[0][i][j] += pow( st[j], 2 )       * pow( r[i], -2 ) * Omega.back() / ( (double) n_steps_rampup );
				//E[1][i][j] += - 2.0 * st[j] * ct[j] * pow( r[i], -2 ) * Omega.back() / ( (double) n_steps_rampup );
				
				
				//--- Version 2: use known exact magnetic field values ---
				E[0][i][j] += factor * B_t_function( r[i], t[j] );
				E[1][i][j] -= factor * B_r_function( r[i], t[j] );
				
				//--- Version 3: use current magnetic field values ---
				//E[0][i][j] += factor * B[1][i][j];
				//E[1][i][j] -= factor * B[0][i][j];
...
void setup_twist_final_Omega(){
	...
	double ratio = 0.99;
	double const_B = - log( 1.0 - ratio ) / (   twist_r_min_99pct - twist_r_min );
	double const_C = - log( 1.0 - ratio ) / ( - twist_r_max_99pct + twist_r_max );
	double const_D = - log( 1.0 - ratio ) / (   twist_t_min_99pct - twist_t_min );
	double const_E = - log( 1.0 - ratio ) / ( - twist_t_max_99pct + twist_t_max );
	
	for( int i=0; i<n_r; i++ ){
		if( ( r[i] >= twist_r_min ) && ( r[i] <= twist_r_max ) ){
			for( int j=0; j<n_t; j++ ){
				if( ( t[j] >= twist_t_min ) && ( t[j] <= twist_t_max ) ){
					
					double smoothing = 1.0;
					smoothing *= 1.0 - exp( - const_B * (   r[i] - twist_r_min ) );
					smoothing *= 1.0 - exp( - const_C * ( - r[i] + twist_r_max ) );
					smoothing *= 1.0 - exp( - const_D * (   t[j] - twist_t_min ) );
					smoothing *= 1.0 - exp( - const_E * ( - t[j] + twist_t_max ) );
					
					twist_final_Omega_values[i][j] = twist_final_Omega * smoothing;
...
void ramp_up_electric_fields_for_twist(){
	if( ( T_index >= T_index_twist_ramp_start ) and ( T_index <= T_index_twist_ramp_stop ) ){
	
		int n_steps_twist = T_index_twist_ramp_stop - T_index_twist_ramp_start + 1;
			...
			if( ( r[i] >= twist_r_min ) && ( r[i] <= twist_r_max ) ){
				for( int j=0; j<n_t; j++ ){
					if( ( t[j] >= twist_t_min ) && ( t[j] <= twist_t_max ) ){
						...
						double factor = twist_final_Omega_values[i][j] * r[i] * st[j] / (double) n_steps_twist;
		...
		for( int i=0; i<n_r; i++ ){
			for( int j=0; j<n_t; j++ ){
				...
				if( is_gridpoint_within_FF_region[i][j] ){
					...
					if( force_free_conditions[i][j] ){
						// Gridpoint is within FF region, and FF conditions satisfied. No need to modify fields.
						were_FF_conditions_applied[i][j] = -1;
					}
					
					else {
						// Gridpoint is within FF region, but FF conditions NOT satisfied. Modify fields.
...
void calculate_electric_and_magnetic_energy(){
	...
	total_energy          = total_electric_energy + total_magnetic_energy;
...
void output_to_csv_profiles( bool condition_override = false ){
	...
	bool condition_T_index_1 = ( T_index >= csv_profiles_write_T_min ) and ( T_index <= csv_profiles_write_T_max );
	bool condition_T_index_2 = ( T_index % csv_profiles_write_freq_T == 0 ) or ( T_index == T_index_max );
	
	if( ( condition_T_index_1 and condition_T_index_2 ) or condition_override ){
...
void output_headers_to_csv_history(){
	...
	output_file_history << "T_index"
						...
						<< ",nans_tot,total_electric_energy,total_magnetic_energy,total_energy,normalised_B_div_volume_integral"
						<< "\n";
...
void output_to_csv_history(){
	...
	if( ( T_index % csv_history_write_freq_T == 0 ) or ( T_index == T_index_max ) ){
		...
		output_file_history <<","<< nans_tot <<","<< total_electric_energy <<","<< total_magnetic_energy <<","<< total_energy <<","<< normalised_B_div_volume_integral << "\n";
...
void output_parameters( std::ostream &os ){
	...
	os << "\nTwist start                :\t" << T_index_twist_ramp_start    <<"\t"<< T_index_twist_ramp_start    * delta_T << "\n";
	os << "Twist stop                   :\t" << T_index_twist_ramp_stop     <<"\t"<< T_index_twist_ramp_stop     * delta_T << "\n";
	os << "Twist magnitude              :\t" << twist_final_Omega << "\n";
	os << "Twist min radius             :\t" << twist_r_min       << "\n";
	os << "Twist ramp    99% radius     :\t" << twist_r_min_99pct << "\n";
	os << "Twist de-ramp 99% radius     :\t" << twist_r_max_99pct << "\n";
	os << "Twist max radius             :\t" << twist_r_max       << "\n";
	os << "Twist min angle (rad)        :\t" << twist_t_min       << "\n";
	os << "Twist ramp    99% angle (rad):\t" << twist_t_min_99pct << "\n";
	os << "Twist de-ramp 99% angle (rad):\t" << twist_t_max_99pct << "\n";
	os << "Twist max angle (rad)        :\t" << twist_t_max       << "\n";
	
	
	os << "\nPrinting values at ( r[" << cout_i <<"], theta[" << cout_j <<"] ) = ( " << r[cout_i] <<", " << t[cout_j] << " ) = ( " << r[cout_i] <<", " << t[cout_j]/pi << " pi )." << "\n";
