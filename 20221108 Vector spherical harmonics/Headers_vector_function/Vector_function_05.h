...
A = [ cos(theta) cos(phi) + 1 ] e_r
  + [ cos(theta) sin(phi) + i cos(phi) ] e_theta
  + [ P_2(cos(theta)) cos(phi) + i sin(theta) sin(phi) ] e_phi,
...
std::complex<double> A_p_function( double t, double p ){
	double re = 0.5*(3*pow(cos(t),2)-1) * cos(p);
	double im = sin(t)*sin(p);
	return std::complex<double> { re, im };
}
