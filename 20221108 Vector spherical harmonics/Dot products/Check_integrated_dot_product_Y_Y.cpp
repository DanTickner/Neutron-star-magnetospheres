...
// g++ Check_integrated_dot_product_Y_Y.cpp -o Check_integrated_dot_product_Y_Y -std=c++11

/*
\int Y_ell^m cdot ( Y_ell'^m' )^* dOmega = delta_{ell,ell'} delta^{m,m'}
*/
		...
		for( int m=-ell; m<=ell; m++ ){
			for( int ell_prime=0; ell_prime<=ell_max; ell_prime++ ){
				for( int m_prime=-ell_prime; m_prime<=ell_prime; m_prime++ ){
					...
					for( int i=0; i<N_theta; i++ ){
						for( int j=0; j<N_phi; j++ ){
							integrand_values[i][j] = dot_product_cart( VSH_Y( theta_list[i], phi_list[j], ell, m ), vector_conj( VSH_Y( theta_list[i], phi_list[j], ell_prime, m_prime ) ) );
						}
					}
