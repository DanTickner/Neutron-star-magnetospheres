...
// g++ Check_integrated_dot_product_mixed_VSHs.cpp -o Check_integrated_dot_product_mixed_VSHs

/*
\int VSH_ell^m cdot ( VSH_ell'^m' )^* dOmega = 0
where the VSHs are different (for example, one is Y and the other is Psi).
To avoid duplication of codes, change the definition of integrand_values[i][j] (line 54) in order to trial different VSH combinations.
*/
		...
		for( int m=-ell; m<=ell; m++ ){
			for( int ell_prime=0; ell_prime<=ell_max; ell_prime++ ){
				for( int m_prime=-ell_prime; m_prime<=ell_prime; m_prime++ ){
					...
					for( int i=0; i<N_theta; i++ ){
						for( int j=0; j<N_phi; j++ ){
							integrand_values[i][j] = dot_product_cart( VSH_Psi( theta_list[i], phi_list[j], ell, m ), vector_conj( VSH_Phi( theta_list[i], phi_list[j], ell_prime, m_prime ) ) );
						}
					}
