...
// g++ Check_dot_product_Y_Y.cpp -o Check_dot_product_Y_Y -std=c++11

/*
Y_ell^m cdot Y_ell'^m' = Y_ell^m Y_ell'^m'
*/
		...
		for( int m=-ell; m<=ell; m++ ){
			for( int ell_prime=0; ell_prime<=ell_max; ell_prime++ ){
				for( int m_prime=-ell_prime; m_prime<=ell_prime; m_prime++ ){
					...
					std::complex<double> dot_product_exact = dot_product_cart( VSH_Y( theta_test, phi_test, ell, m ), VSH_Y( theta_test, phi_test, ell_prime, m_prime ) );
					std::complex<double> dot_product_guess = ylm( theta_test, phi_test, ell, m ) * ylm( theta_test, phi_test, ell_prime, m_prime );
				...
				for( int m_prime=-ell_prime; m_prime<=ell_prime; m_prime++ ){
					...
					for( int i=0; i<N_theta; i++ ){
						for( int j=0; j<N_phi; j++ ){
							...
							std::complex<double> dot_product_exact = dot_product_cart( VSH_Y( theta_list[i], phi_list[j], ell, m ), VSH_Y( theta_list[i], phi_list[j], ell_prime, m_prime ) );
							std::complex<double> dot_product_guess = ylm( theta_list[i], phi_list[j], ell, m ) * ylm( theta_list[i], phi_list[j], ell_prime, m_prime );
