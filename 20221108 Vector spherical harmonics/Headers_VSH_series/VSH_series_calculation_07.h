// VSH_series_calculation_07.h
// Calculate the VSH series coefficients for a vector A with no r-dependence.
// Calculate the m<0 and m>0 terms simultaneously.
...
std::vector< std::vector< std::complex<double> > > generate_vsh_series_C1( std::vector< std::vector< std::vector< std::complex<double> > > > A, std::vector<double> t_list, std::vector<double> p_list, int ell_max ){
	...
	for( int ell=1; ell<=ell_max; ell++ ){
		...
		for( int m=1; m<=ell; m++ ){
			...
			for( int i=0; i<N_t; i++ ){
				for( int j=0; j<N_p; j++ ){
					integrand_plus [i][j] = A[1][i][j] * std::conj( ylm_dtheta( t_list[i], p_list[j], ell, m ) ) + A[2][i][j] * std::complex<double>{0,(double)-m} * std::conj( ylm_over_sintheta( t_list[i], p_list[j], ell, m ) );
					integrand_minus[i][j] = A[1][i][j] *            ylm_dtheta( t_list[i], p_list[j], ell, m )   + A[2][i][j] * std::complex<double>{0,(double) m} *            ylm_over_sintheta( t_list[i], p_list[j], ell, m );
				}
			}
