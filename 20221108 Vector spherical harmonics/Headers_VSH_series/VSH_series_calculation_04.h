// VSH_series_calculation_04.h
// Calculate the VSH series coefficients for a vector A with no r-dependence.
// Calculate the m-component of the integral only if needed, eliminating the need to call Ylm_over_sintheta() and enabling future use of precalculated spherical harmonics.
	...
	for( int ell=1; ell<=ell_max; ell++ ){
		...
		for( int m=-ell; m<=ell; m++ ){
			...
			if( m != 0 ){
				for( int i=0; i<N_t; i++ ){
					for( int j=0; j<N_p; j++ ){
						integrand[i][j] = A[2][i][j] * std::complex<double>{0,-(double)m} * std::conj( ylm_over_sintheta( t_list[i], p_list[j], ell, m ) );
					}
				}
				coeff += integral_spherical_surface( integrand, t_list, p_list );
			}
