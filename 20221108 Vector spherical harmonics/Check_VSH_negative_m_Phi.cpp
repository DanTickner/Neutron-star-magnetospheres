...
// g++ Check_VSH_negative_m_Phi.cpp -o Check_VSH_negative_m_Phi

/*
Check the following relation:
\Psi_ell^{-}m = (-1)^m e^{-i2mphi} ( dY_ell^m/dtheta e_theta - 1/sin(theta) dY_ell^m/dphi e_phi ).
*/
...
int main(){
	...
	for( int ell=0; ell<=ell_max; ell++ ){
		for( int m=0; m<=ell; m++ ){
			...
			std::vector< std::complex<double> > VSH_test( 3 );
			VSH_test[0] = 0;
			VSH_test[1] = std::complex<double>{0,1}*(m/sin(theta_list[i_test]))*ylm(theta_list[i_test],phi_list[j_test],ell,m);
			VSH_test[2] = ylm_dtheta(theta_list[i_test],phi_list[j_test],ell,m);
			VSH_test = scalar_times_vector_cart( pow(-1,m)*exp(std::complex<double>{0,-2*m*phi_list[j_test]}), VSH_test );

			std::vector< std::complex<double> > VSH_exact = VSH_Phi( theta_list[i_test], phi_list[j_test], ell, -m );
