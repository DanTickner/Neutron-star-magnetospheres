...
// g++ Fit_VSH_series_no_r_dependence_201.cpp -o Fit_VSH_series_no_r_dependence_201
...
#include "../../Generate_Associated_Legendre_Functions.h"	// Needed for v3 of VSH series evaluation function
#include "../Headers_vector_function/Vector_function_05_ell_equals_0.h"
...
int main(){
	...
	/*//--- V2 (Book 11, p54) ---
	double theta = t_list[i_test];
	double phi   = p_list[j_test];
	// r-component
	for( int ell=0; ell<=ell_max; ell++ ){
		for( int m=-ell; m<=ell; m++ ){
			A_test_calc[0] += coeffs_VSH_series[0][ell][ell+m] * ylm(theta,phi,ell,m);
		}
	}
	...
	for( int ell=1; ell<=ell_max; ell++ ){
		for( int m=-ell; m<=ell; m++ ){
			A_test_calc[1] += coeffs_VSH_series[1][ell][ell+m] * ylm_dtheta(theta,phi,ell,m) - coeffs_VSH_series[2][ell][ell+m] * ylm_dphi(theta,phi,ell,m) * pow(sin(theta),-1);
			A_test_calc[2] += coeffs_VSH_series[2][ell][ell+m] * ylm_dtheta(theta,phi,ell,m) + coeffs_VSH_series[1][ell][ell+m] * ylm_dphi(theta,phi,ell,m) * pow(sin(theta),-1);
		}
	}*/
	
	//--- V3 (Check expression for m=0 is correct; Book 11, p58) ---
	...
	for( int ell=0; ell<=ell_max; ell++ ){
		for( int m=-ell; m<=ell; m++ ){
			if( m == 0 ){
				A_test_calc[0] += coeffs_VSH_series[0][ell][ell] * sqrt( ( 2.0*ell+1.0 ) * pow(4*pi,-1) ) * associated_legendre_function(cos(theta),ell,0);
			} else {
				A_test_calc[0] += coeffs_VSH_series[0][ell][ell+m] * ylm(theta,phi,ell,m);
			}
		}
	...
	for( int ell=1; ell<=ell_max; ell++ ){
		for( int m=-ell; m<=ell; m++ ){
			if( m == 0 ){
				double factor = sqrt( ( 2.0*ell+1.0 ) * pow(4*pi,-1) ) * associated_legendre_function(cos(theta),ell,1);
				A_test_calc[1] += coeffs_VSH_series[1][ell][ell] * factor;
				A_test_calc[2] += coeffs_VSH_series[2][ell][ell] * factor;
			} else {
				A_test_calc[1] += coeffs_VSH_series[1][ell][ell+m] * ylm_dtheta(theta,phi,ell,m) - coeffs_VSH_series[2][ell][ell+m] * ylm_dphi(theta,phi,ell,m) * pow(sin(theta),-1);
				A_test_calc[2] += coeffs_VSH_series[2][ell][ell+m] * ylm_dtheta(theta,phi,ell,m) + coeffs_VSH_series[1][ell][ell+m] * ylm_dphi(theta,phi,ell,m) * pow(sin(theta),-1);
			}
