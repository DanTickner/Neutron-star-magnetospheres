...
// g++ Fit_VSH_series_no_r_dependence_06.cpp -o Fit_VSH_series_no_r_dependence_06

/*
Fit a vector spherical harmonic series to a vector A(theta,phi).
Precalculate the spherical harmonics for the VSHs.
*/
...
#include "../Headers_VSH_series/VSH_series_calculation_18.h"	// Works from _14 inclusive.
#include "../Headers_VSH_series/VSH_series_evaluation_10.h"		// Works from _05 inclusive.
...
int main(){
	...
	std::vector< std::vector< std::vector< std::vector< std::complex<double> > > > > ylm_vals              ( ell_max+1, std::vector< std::vector< std::vector< std::complex<double> > > > ( 2*ell_max+1, std::vector< std::vector< std::complex<double> > > ( N_t, std::vector< std::complex<double> > ( N_p ) ) ) );
	std::vector< std::vector< std::vector< std::vector< std::complex<double> > > > > ylm_dtheta_vals       ( ell_max+1, std::vector< std::vector< std::vector< std::complex<double> > > > ( 2*ell_max+1, std::vector< std::vector< std::complex<double> > > ( N_t, std::vector< std::complex<double> > ( N_p ) ) ) );
	std::vector< std::vector< std::vector< std::vector< std::complex<double> > > > > ylm_over_sintheta_vals( ell_max+1, std::vector< std::vector< std::vector< std::complex<double> > > > ( 2*ell_max+1, std::vector< std::vector< std::complex<double> > > ( N_t, std::vector< std::complex<double> > ( N_p ) ) ) );
	...
	//----- Precalculate the Y_ell^m, Y_ell^m_dtheta and Y_ell^m/sin(theta) required by the fit -----
	for( int ell=0; ell<=ell_max; ell++ ){
		for( int m=-ell; m<=ell; m++ ){
			for( int i=0; i<N_t; i++ ){
				for( int j=0; j<N_p; j++ ){
					ylm_vals              [ell][ell+m][i][j] = ylm              ( t_list[i], p_list[j], ell, m );
					ylm_dtheta_vals       [ell][ell+m][i][j] = ylm_dtheta       ( t_list[i], p_list[j], ell, m );
					ylm_over_sintheta_vals[ell][ell+m][i][j] = ylm_over_sintheta( t_list[i], p_list[j], ell, m );
				}
			}
		}
	}
	...
	//std::vector< std::vector< std::vector< std::complex<double> > > > coeffs_VSH_series = generate_vsh_series( A, t_list, p_list, ell_max );
	std::vector< std::vector< std::vector< std::complex<double> > > > coeffs_VSH_series = generate_vsh_series( A, t_list, p_list, ylm_vals, ylm_dtheta_vals, ylm_over_sintheta_vals );
	...
	std::vector< std::complex<double> > A_test_calc  = evaluate_vsh_series( t_list[test_t_i], p_list[test_p_j], coeffs_VSH_series );
	...
	for( int ell=0; ell<=ell_max; ell++ ){
		for( int m=-ell; m<=ell; m++ ){
			A_test_r += coeffs_VSH_series[0][ell][ell+m] * ylm( t_list[test_t_i], p_list[test_p_j], ell, m );
		}
	}
