...
// g++ Cross_product_Y_Psi.cpp -o Cross_product_Y_Psi

/*
Cross products of vector spherical harmonics.
Y_{ell_1}^{m_1} x Psi_{ell_2}^{m_2}
= -im_2/sin(theta) Y_{ell_1}^{m_1} Y_{ell_2}^{m_2} e_theta + Y_{ell_1}^{m_1} d/dtheta Y_{ell_2}^{m_2} e_phi
= Y_{ell_1}^{m_1} Phi_{ell_2}^{m_2}.
*/
...
std::vector< std::complex<double> > exact( double theta, double phi, int ell_1, int m_1, int ell_2, int m_2 ){
	return cross_product( VSH_Y( theta, phi, ell_1, m_1 ), VSH_Psi( theta, phi, ell_2, m_2 ) );
}
...
std::vector< std::complex<double> > test ( double theta, double phi, int ell_1, int m_1, int ell_2, int m_2 ){
	...
	if( m_2 != 0 ){
		ret[1] = ylm(theta,phi,ell_1,m_1) * std::complex<double>{0,(double)-m_2} * ylm_over_sintheta(theta,phi,ell_2,m_2);
	}
	ret[2] = ylm(theta,phi,ell_1,m_1) * ylm_dtheta(theta,phi,ell_2,m_2);
	return ret;*/
	
	//--- V2 ---
	return scalar_times_vector( ylm(theta,phi,ell_1,m_1), VSH_Phi(theta,phi,ell_2,m_2) );
}