...
// g++ Cross_product_of_VSH_series_103a.cpp -o Cross_product_of_VSH_series_103a
...
Potentially the double sum means our expression for the product of two summations doesn't carry over. Remove it in this side version.
*/
...
int main(){
	...
	//--- V1 ---
	std::complex<double> sum_1 = 0;
	for( int L1=0; L1<=ell_max; L1++ ){
		int M1 = 0;
		sum_1 += coeffs_A[0][L1][L1+M1] * ylm(t,p,L1,M1);
	}
	
	std::vector< std::complex<double> > sum_2 { 0, 0, 0 };
	for( int L2=0; L2<=ell_max; L2++ ){
		int M2 = 0;
		sum_2 = vector_sum( sum_2, scalar_times_vector( coeffs_B[1][L2][L2+M2], VSH_Phi(t,p,L2,M2) ) );
	}
	cross_product_from_VSH_coeffs_01a = scalar_times_vector( sum_1, sum_2 );
	
	
	//--- V2 (doesn't work) ---
	/*std::vector< std::complex<double> > sum_1 { 0, 0, 0 };
	std::vector< std::complex<double> > sum_2 { 0, 0, 0 };
	for( int L1=0; L1<=ell_max; L1++ ){
		int M1 = 0;
		sum_1 = vector_sum( sum_1, scalar_times_vector( coeffs_A[0][L1][L1+M1] * ylm(t,p,L1,M1) * coeffs_B[1][L1][L1+M1], VSH_Phi(t,p,L1,M1) ) );
	
		for( int L2=L1+1; L2<=ell_max; L2++ ){
			int M2 = 0;
			sum_2 = vector_sum( sum_2, scalar_times_vector( coeffs_A[0][L1][L1+M1] * ylm(t,p,L1,M1) * coeffs_B[1][L2][L2+M2], VSH_Phi(t,p,L2,M2) ) );
			sum_2 = vector_sum( sum_2, scalar_times_vector( coeffs_A[0][L2][L2+M2] * ylm(t,p,L2,M2) * coeffs_B[1][L1][L1+M1], VSH_Phi(t,p,L1,M1) ) );
		}
	}
	cross_product_from_VSH_coeffs_01a = vector_sum( sum_1, sum_2 );
	print_vector( sum_1 );print_vector( sum_2 );*/
	
	
	//--- V3 ---
	/*std::vector< std::complex<double> > sum_Phi1 { 0, 0, 0 };
	for( int L1=0; L1<=ell_max; L1++ ){
		for( int M1=-L1; M1<=L1; M1++ ){
			std::complex<double> sum_1 = 0;
			for( int L2=L1+1; L2<=ell_max; L2++ ){
				for( int M2=-L2; M2<=L2; M2++ ){
					sum_1 += coeffs_A[0][L2][L2+M2] * ylm(t,p,L2,M2);
				}
			}
			sum_1 = sum_1 * coeffs_B[1][L1][L1+M1] + coeffs_A[0][L1][L1+M1] * coeffs_B[0][L1][L1+M1] * ylm(t,p,L1,M1);
			sum_Phi1 = vector_sum( sum_Phi1, scalar_times_vector( sum_1, VSH_Phi(t,p,L1,M1) ) );
		}
	}*/
	...
	int w = 30;
	//print_vector_setw( cross_product_from_VSH_coeffs_01_Psi1, w, "Y_1   x Psi_2 (Phi_1 term)" );
	//print_vector_setw( cross_product_from_VSH_coeffs_01_Psi2, w, "Y_1   x Psi_2 (Phi_2 term)" );
