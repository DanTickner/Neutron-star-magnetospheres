...
// g++ Cross_product_of_VSH_series_203_2.cpp -o Cross_product_of_VSH_series_203_2
...
int main(){
	...
	std::vector< std::complex<double> > cross_product_from_VSH_coeffs_02a { 0, 0, 0 };	// until calculation correct
	
	
	//----- Calculate _02 sum -----
	
	//--- V3 ---
	std::vector< std::complex<double> > cross_product_from_VSH_coeffs_02a_Phi_k1_part { 0, 0, 0 };
	for( int k1=0; k1<=k_max; k1++ ){
		...
		sum_over_k2 *= - coeffs_B[2][L1][L1+M1];
		cross_product_from_VSH_coeffs_02a_Phi_k1_part = vector_sum( cross_product_from_VSH_coeffs_02a_Phi_k1_part, scalar_times_vector( sum_over_k2, VSH_Psi(t,p,L1,M1) ) );
	}
	
	std::vector< std::complex<double> > cross_product_from_VSH_coeffs_02a_Phi_k2_part { 0, 0, 0 };
	for( int k1=0; k1<=k_max; k1++ ){
		int L1 = sqrt(k1);
		int M1 = k1 - L1*(L1+1);
		...
		for( int k2=k1+1; k2<=k_max; k2++ ){	
			int L2 = sqrt(k2);
			int M2 = k2 - L2*(L2+1);
			sum_over_k2 = vector_sum( sum_over_k2, scalar_times_vector( coeffs_B[2][L2][L2+M2], VSH_Psi(t,p,L2,M2) ) );
		}
		sum_over_k2 = scalar_times_vector( - coeffs_A[0][L1][L1+M1] * ylm(t,p,L1,M1), sum_over_k2 );
		cross_product_from_VSH_coeffs_02a_Phi_k2_part = vector_sum( cross_product_from_VSH_coeffs_02a_Phi_k2_part, sum_over_k2 );
	}
	
	cross_product_from_VSH_coeffs_02a = vector_sum( cross_product_from_VSH_coeffs_02a, cross_product_from_VSH_coeffs_02a_Phi_k1_part );
	cross_product_from_VSH_coeffs_02a = vector_sum( cross_product_from_VSH_coeffs_02a, cross_product_from_VSH_coeffs_02a_Phi_k2_part );
	...
	print_vector_setw( cross_product_from_VSH_coeffs_02a    , w, "Y_1   x Phi_2 (added)" );
