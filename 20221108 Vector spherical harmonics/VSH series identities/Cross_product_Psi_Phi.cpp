...
// g++ Cross_product_Psi_Phi.cpp -o Cross_product_Psi_Phi

/*
Cross products of vector spherical harmonics.
Psi_{ell_1}^{m_1} x Phi_{ell_2}^{m_2}
= ( d/dtheta Y_{ell_1}^{m_1} d/dtheta Y_{ell_2}^{m_2} - m_1m_2/sin^2(theta) Y_{ell_1}^{m_1} Y_{ell_2}^{m_2} ) e_r
= ( Psi_{ell_1}^{m_1} dot Psi_{ell_2}^{m_2} ) e_r = ( Phi_{ell_1}^{m_1} dot Phi_{ell_2}^{m_2} ) e_r.
*/
				...
				for( int m_2=-ell_2; m_2<=ell_2; m_2++ ){
					...
					for( int i=0; i<N_t; i++ ){
						for( int j=0; j<N_p; j++ ){
							...
							for( int c=0; c<3; c++ ){
								if( !std::isfinite( abs(LHS[c]) ) ){
									std::cout << "LHS[" << c << "] is " << LHS[c] << std::endl;
								}
								if( !std::isfinite( abs(RHS[c]) ) ){
									std::cout << "\t\tRHS[" << c << "] is " << RHS[c] << std::endl;	// Tabs to make it obvious on a fast-scrolling output.
								}
							}
							for( int c=0; c<3; c++ ){
...
std::vector< std::complex<double> > exact( double theta, double phi, int ell_1, int m_1, int ell_2, int m_2 ){
	return cross_product( VSH_Psi( theta, phi, ell_1, m_1 ), VSH_Phi( theta, phi, ell_2, m_2 ) );
}
...
std::vector< std::complex<double> > test ( double theta, double phi, int ell_1, int m_1, int ell_2, int m_2 ){
	...
	ret[0] = ylm_dtheta(theta,phi,ell_1,m_1) * ylm_dtheta(theta,phi,ell_2,m_2);
	if( ( m_1 != 0 ) && ( m_2 != 0 ) ){
		ret[0] -= (double)( m_1 * m_2 ) * ylm_over_sintheta(theta,phi,ell_1,m_1) * ylm_over_sintheta(theta,phi,ell_2,m_2);
	}
	return ret;*/
	
	//--- V2 ---
	//return std::vector< std::complex<double> >{ dot_product( VSH_Psi(theta,phi,ell_1,m_1), VSH_Psi(theta,phi,ell_2,m_2) ), 0, 0 };
	
	//--- V3 ---
	return std::vector< std::complex<double> >{ dot_product( VSH_Phi(theta,phi,ell_1,m_1), VSH_Phi(theta,phi,ell_2,m_2) ), 0, 0 };
}