...
// g++ Cross_product_of_VSH_series_203_4578.cpp -o Cross_product_of_VSH_series_203_4578
...
Calculate _11, _12, _21 and _22 sums separately.
*/
...
int main(){
	...
	std::vector< std::complex<double> > cross_product_from_VSH_coeffs_11_12_21_22  { 0, 0, 0 };	// Add the correct values together for comparison
	std::vector< std::complex<double> > cross_product_from_VSH_coeffs_11_12_21_22a { 0, 0, 0 };	// until calculation correct
	
	
	//----- Calculate _11, _12, _21 and _22 sums -----
	//--- V0 (write them in this loop) ---
	/*for( int k1=0; k1<=k_max; k1++ ){
		...
		for( int k2=0; k2<=k_max; k2++ ){	
			...
			std::complex<double> coeff_11 = coeffs_A[1][L1][L1+M1] * coeffs_B[1][L2][L2+M2];
			std::complex<double> coeff_12 = coeffs_A[1][L1][L1+M1] * coeffs_B[2][L2][L2+M2];
			std::complex<double> coeff_21 = coeffs_A[2][L1][L1+M1] * coeffs_B[1][L2][L2+M2];
			std::complex<double> coeff_22 = coeffs_A[2][L1][L1+M1] * coeffs_B[2][L2][L2+M2];
			std::vector< std::complex<double> > cross_product_11 = cross_product( VSH_Psi(t,p,L1,M1), VSH_Psi(t,p,L2,M2) );
			std::vector< std::complex<double> > cross_product_12 = cross_product( VSH_Psi(t,p,L1,M1), VSH_Phi(t,p,L2,M2) );
			std::vector< std::complex<double> > cross_product_21 = cross_product( VSH_Phi(t,p,L1,M1), VSH_Psi(t,p,L2,M2) );
			std::vector< std::complex<double> > cross_product_22 = cross_product( VSH_Phi(t,p,L1,M1), VSH_Phi(t,p,L2,M2) );
			cross_product_from_VSH_coeffs_11_12_21_22a = vector_sum( cross_product_from_VSH_coeffs_11_12_21_22a, scalar_times_vector( coeff_11, cross_product_11 ) );
			cross_product_from_VSH_coeffs_11_12_21_22a = vector_sum( cross_product_from_VSH_coeffs_11_12_21_22a, scalar_times_vector( coeff_12, cross_product_12 ) );
			cross_product_from_VSH_coeffs_11_12_21_22a = vector_sum( cross_product_from_VSH_coeffs_11_12_21_22a, scalar_times_vector( coeff_21, cross_product_21 ) );
			cross_product_from_VSH_coeffs_11_12_21_22a = vector_sum( cross_product_from_VSH_coeffs_11_12_21_22a, scalar_times_vector( coeff_22, cross_product_22 ) );
		}
	}*/
	
	//--- V1 (Equivalent cross products) ---
	/*for( int k1=0; k1<=k_max; k1++ ){
		int L1 = sqrt(k1);
		int M1 = k1 - L1*(L1+1);
		for( int k2=0; k2<=k_max; k2++ ){	
			...
			std::complex<double> coeff_11 = coeffs_A[1][L1][L1+M1] * coeffs_B[1][L2][L2+M2];
			std::complex<double> coeff_12 = coeffs_A[1][L1][L1+M1] * coeffs_B[2][L2][L2+M2];
			std::complex<double> coeff_21 = coeffs_A[2][L1][L1+M1] * coeffs_B[1][L2][L2+M2] * -1.0;
			std::complex<double> coeff_22 = coeffs_A[2][L1][L1+M1] * coeffs_B[2][L2][L2+M2];
			std::vector< std::complex<double> > cross_product_11 = cross_product( VSH_Psi(t,p,L1,M1), VSH_Psi(t,p,L2,M2) );
			std::vector< std::complex<double> > cross_product_12 = cross_product( VSH_Psi(t,p,L1,M1), VSH_Phi(t,p,L2,M2) );
			cross_product_from_VSH_coeffs_11_12_21_22a = vector_sum( cross_product_from_VSH_coeffs_11_12_21_22a, scalar_times_vector( coeff_11, cross_product_11 ) );
			cross_product_from_VSH_coeffs_11_12_21_22a = vector_sum( cross_product_from_VSH_coeffs_11_12_21_22a, scalar_times_vector( coeff_12, cross_product_12 ) );
			cross_product_from_VSH_coeffs_11_12_21_22a = vector_sum( cross_product_from_VSH_coeffs_11_12_21_22a, scalar_times_vector( coeff_21, cross_product_12 ) );
			cross_product_from_VSH_coeffs_11_12_21_22a = vector_sum( cross_product_from_VSH_coeffs_11_12_21_22a, scalar_times_vector( coeff_22, cross_product_11 ) );
		}
	}*/
	...
	for( int k1=0; k1<=k_max; k1++ ){
		...
		for( int k2=0; k2<=k_max; k2++ ){	
			...
			std::complex<double> coeff_Psi_cross_Psi = coeffs_A[1][L1][L1+M1] * coeffs_B[1][L2][L2+M2] + coeffs_A[2][L1][L1+M1] * coeffs_B[2][L2][L2+M2];
			std::complex<double> coeff_Psi_cross_Phi = coeffs_A[1][L1][L1+M1] * coeffs_B[2][L2][L2+M2] - coeffs_A[2][L1][L1+M1] * coeffs_B[1][L2][L2+M2];
			std::vector< std::complex<double> > Psi_cross_Psi = cross_product( VSH_Psi(t,p,L1,M1), VSH_Psi(t,p,L2,M2) );
			std::vector< std::complex<double> > Psi_cross_Phi = cross_product( VSH_Psi(t,p,L1,M1), VSH_Phi(t,p,L2,M2) );
			cross_product_from_VSH_coeffs_11_12_21_22a = vector_sum( cross_product_from_VSH_coeffs_11_12_21_22a, scalar_times_vector( coeff_Psi_cross_Psi, Psi_cross_Psi ) );
			cross_product_from_VSH_coeffs_11_12_21_22a = vector_sum( cross_product_from_VSH_coeffs_11_12_21_22a, scalar_times_vector( coeff_Psi_cross_Phi, Psi_cross_Phi ) );
		}
	}
	...
	cross_product_from_VSH_coeffs_11_12_21_22 = vector_sum( cross_product_from_VSH_coeffs_11_12_21_22, cross_product_from_VSH_coeffs_11 );
	cross_product_from_VSH_coeffs_11_12_21_22 = vector_sum( cross_product_from_VSH_coeffs_11_12_21_22, cross_product_from_VSH_coeffs_12 );
	cross_product_from_VSH_coeffs_11_12_21_22 = vector_sum( cross_product_from_VSH_coeffs_11_12_21_22, cross_product_from_VSH_coeffs_21 );
	cross_product_from_VSH_coeffs_11_12_21_22 = vector_sum( cross_product_from_VSH_coeffs_11_12_21_22, cross_product_from_VSH_coeffs_22 );
	
	print_vector_setw( cross_product_from_VSH_coeffs_11_12_21_22 , w, "Y-terms (exact)" );
	print_vector_setw( cross_product_from_VSH_coeffs_11_12_21_22a, w, "Y-terms (added)" );
