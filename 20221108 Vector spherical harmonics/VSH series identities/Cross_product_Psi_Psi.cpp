...
// g++ Cross_product_Psi_Psi.cpp -o Cross_product_Psi_Psi

/*
Cross products of vector spherical harmonics.
Psi_{ell_1}^{m_1} x Psi_{ell_2}^{m_2}
= ( i m_2 / sin(theta) Y_{ell_2}^{m_2} d/dtheta Y_{ell_1}^{m_1} - i m_1 / sin(theta) Y_{ell_1}^{m_1} d/dtheta Y_{ell_2}^{m_2} ) e_r
= ( Phi_{ell_1}^{m_1} dot Psi_{ell_2}^{m_2} ) e_r.
*/
...
std::vector< std::complex<double> > exact( double theta, double phi, int ell_1, int m_1, int ell_2, int m_2 ){
	return cross_product( VSH_Psi( theta, phi, ell_1, m_1 ), VSH_Psi( theta, phi, ell_2, m_2 ) );
}
...
std::vector< std::complex<double> > test ( double theta, double phi, int ell_1, int m_1, int ell_2, int m_2 ){
	...
	if( m_2 != 0 ){
		ret[0] += std::complex<double>{0,(double)m_2} * ylm_over_sintheta(theta,phi,ell_2,m_2) * ylm_dtheta(theta,phi,ell_1,m_1);
	}
	if( m_1 != 0 ){
		ret[0] -= std::complex<double>{0,(double)m_1} * ylm_over_sintheta(theta,phi,ell_1,m_1) * ylm_dtheta(theta,phi,ell_2,m_2);
	}
	return ret;*/
	
	//--- V2 ---
	return std::vector< std::complex<double> >{ dot_product( VSH_Phi(theta,phi,ell_1,m_1), VSH_Psi(theta,phi,ell_2,m_2) ), 0, 0 };
	
	//--- V3 ---
	//return cross_product( VSH_Phi( theta, phi, ell_1, m_1 ), VSH_Phi( theta, phi, ell_2, m_2 ) );
}