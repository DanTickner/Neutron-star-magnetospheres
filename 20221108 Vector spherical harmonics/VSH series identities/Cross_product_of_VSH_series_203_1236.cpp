...
// g++ Cross_product_of_VSH_series_203_1236.cpp -o Cross_product_of_VSH_series_203_1236
...
Calculate _01, _02, _10 and _20 sums separately.
*/
...
int main(){
	...
	std::vector< std::complex<double> > cross_product_from_VSH_coeffs_01_02_10_20  { 0, 0, 0 };	// Add the correct values together for comparison
	std::vector< std::complex<double> > cross_product_from_VSH_coeffs_01_02_10_20a { 0, 0, 0 };	// until calculation correct
	
	
	//----- Calculate _01, _02, _10 and _20 sums -----
	for( int k1=0; k1<=k_max; k1++ ){
		int L1 = sqrt(k1);
		int M1 = k1 - L1*(L1+1);
		
		std::complex<double> sum_for_Psi_k1 = 0;
		std::complex<double> sum_for_Phi_k1 = 0;
		for( int k2=k1; k2<=k_max; k2++ ){	
			int L2 = sqrt(k2);
			int M2 = k2 - L2*(L2+1);
			sum_for_Psi_k1 += ylm(t,p,L2,M2) * ( coeffs_A[2][L1][L1+M1] * coeffs_B[0][L2][L2+M2] - coeffs_A[0][L2][L2+M2] * coeffs_B[2][L1][L1+M1] );
			sum_for_Phi_k1 += ylm(t,p,L2,M2) * ( coeffs_A[0][L2][L2+M2] * coeffs_B[1][L1][L1+M1] - coeffs_A[1][L1][L1+M1] * coeffs_B[0][L2][L2+M2] );
		}
		cross_product_from_VSH_coeffs_01_02_10_20a = vector_sum( cross_product_from_VSH_coeffs_01_02_10_20a, scalar_times_vector( sum_for_Psi_k1, VSH_Psi(t,p,L1,M1) ) );
		cross_product_from_VSH_coeffs_01_02_10_20a = vector_sum( cross_product_from_VSH_coeffs_01_02_10_20a, scalar_times_vector( sum_for_Phi_k1, VSH_Phi(t,p,L1,M1) ) );
		
		std::vector< std::complex<double> > sum_over_k2 { 0, 0, 0 };
		for( int k2=k1+1; k2<=k_max; k2++ ){	
			int L2 = sqrt(k2);
			int M2 = k2 - L2*(L2+1);
			std::complex<double> coeff_for_Psi_k2 = coeffs_A[2][L2][L2+M2] * coeffs_B[0][L1][L1+M1] - coeffs_A[0][L1][L1+M1] * coeffs_B[2][L2][L2+M2];
			std::complex<double> coeff_for_Phi_k2 = coeffs_A[0][L1][L1+M1] * coeffs_B[1][L2][L2+M2] - coeffs_A[1][L2][L2+M2] * coeffs_B[0][L1][L1+M1];
			sum_over_k2 = vector_sum( sum_over_k2, scalar_times_vector( coeff_for_Psi_k2, VSH_Psi(t,p,L2,M2) ) );
			sum_over_k2 = vector_sum( sum_over_k2, scalar_times_vector( coeff_for_Phi_k2, VSH_Phi(t,p,L2,M2) ) );
		}
		cross_product_from_VSH_coeffs_01_02_10_20a = vector_sum( cross_product_from_VSH_coeffs_01_02_10_20a, scalar_times_vector( ylm(t,p,L1,M1), sum_over_k2 ) );
	}
	...
	cross_product_from_VSH_coeffs_01_02_10_20 = vector_sum( cross_product_from_VSH_coeffs_01_02_10_20, cross_product_from_VSH_coeffs_01 );
	cross_product_from_VSH_coeffs_01_02_10_20 = vector_sum( cross_product_from_VSH_coeffs_01_02_10_20, cross_product_from_VSH_coeffs_02 );
	cross_product_from_VSH_coeffs_01_02_10_20 = vector_sum( cross_product_from_VSH_coeffs_01_02_10_20, cross_product_from_VSH_coeffs_10 );
	cross_product_from_VSH_coeffs_01_02_10_20 = vector_sum( cross_product_from_VSH_coeffs_01_02_10_20, cross_product_from_VSH_coeffs_20 );
	
	print_vector_setw( cross_product_from_VSH_coeffs_01_02_10_20 , w, "Y-terms (exact)" );
	print_vector_setw( cross_product_from_VSH_coeffs_01_02_10_20a, w, "Y-terms (added)" );
