...
// g++ Cross_product_of_VSH_series_203_4.cpp -o Cross_product_of_VSH_series_203_4
...
int main(){
	...
	std::vector< std::complex<double> > cross_product_from_VSH_coeffs_11a { 0, 0, 0 };	// until calculation correct
	
	
	//----- Calculate _11 sum -----
	
	//--- V0 (just separate from main loop) ---
	/*for( int k1=0; k1<=k_max; k1++ ){
		...
		for( int k2=0; k2<=k_max; k2++ ){	
			...
			cross_product_from_VSH_coeffs_11a = vector_sum( cross_product_from_VSH_coeffs_11a, scalar_times_vector( Ar12[1] * Br12[1], cross_product( Psi1, Psi2 ) ) );
		}
	}*/
	
	//--- V1 (Check that we can perform the sums first and then the cross product) ---
	/*std::vector< std::complex<double> > sum_over_Psi_k1 { 0, 0, 0 };
	for( int k1=0; k1<=k_max; k1++ ){
		int L1 = sqrt(k1);
		int M1 = k1 - L1*(L1+1);
		sum_over_Psi_k1 = vector_sum( sum_over_Psi_k1, scalar_times_vector( coeffs_A[1][L1][L1+M1], VSH_Psi(t,p,L1,M1) ) );
	}
	
	std::vector< std::complex<double> > sum_over_Psi_k2 { 0, 0, 0 };
	for( int k2=0; k2<=k_max; k2++ ){
		int L2 = sqrt(k2);
		int M2 = k2 - L2*(L2+1);
		sum_over_Psi_k2 = vector_sum( sum_over_Psi_k2, scalar_times_vector( coeffs_B[1][L2][L2+M2], VSH_Psi(t,p,L2,M2) ) );
	}
	
	cross_product_from_VSH_coeffs_11a = cross_product( sum_over_Psi_k1, sum_over_Psi_k2 );*/
	...
	for( int k1=0; k1<=k_max; k1++ ){
		...
		for( int k2=k1+1; k2<=k_max; k2++ ){	
			int L2 = sqrt(k2);
			int M2 = k2 - L2*(L2+1);
			std::complex<double> coeff = coeffs_A[1][L1][L1+M1] * coeffs_B[1][L2][L2+M2] - coeffs_A[1][L2][L2+M2] * coeffs_B[1][L1][L1+M1];
			cross_product_from_VSH_coeffs_11a = vector_sum( cross_product_from_VSH_coeffs_11a, scalar_times_vector( coeff, cross_product( VSH_Psi(t,p,L1,M1), VSH_Psi(t,p,L2,M2) ) ) );
		}
	}
			
	
	//--- V3 ---
	/*std::vector< std::complex<double> > cross_product_from_VSH_coeffs_20a_Phi_k1_part { 0, 0, 0 };
	for( int k1=0; k1<=k_max; k1++ ){
		int L1 = sqrt(k1);
		int M1 = k1 - L1*(L1+1);
		...
		for( int k2=k1; k2<=k_max; k2++ ){	
			int L2 = sqrt(k2);
			int M2 = k2 - L2*(L2+1);
			sum_over_k2 += coeffs_B[0][L2][L2+M2] * ylm(t,p,L2,M2);
		}
		sum_over_k2 *= coeffs_A[2][L1][L1+M1];
		cross_product_from_VSH_coeffs_20a_Phi_k1_part = vector_sum( cross_product_from_VSH_coeffs_20a_Phi_k1_part, scalar_times_vector( sum_over_k2, VSH_Psi(t,p,L1,M1) ) );
	}
	
	std::vector< std::complex<double> > cross_product_from_VSH_coeffs_20a_Phi_k2_part { 0, 0, 0 };
	for( int k1=0; k1<=k_max; k1++ ){
		int L1 = sqrt(k1);
		int M1 = k1 - L1*(L1+1);
		...
		for( int k2=k1+1; k2<=k_max; k2++ ){	
			int L2 = sqrt(k2);
			int M2 = k2 - L2*(L2+1);
			sum_over_k2 = vector_sum( sum_over_k2, scalar_times_vector( coeffs_A[2][L2][L2+M2], VSH_Psi(t,p,L2,M2) ) );
		}
		sum_over_k2 = scalar_times_vector( coeffs_B[0][L1][L1+M1] * ylm(t,p,L1,M1), sum_over_k2 );
		cross_product_from_VSH_coeffs_20a_Phi_k2_part = vector_sum( cross_product_from_VSH_coeffs_20a_Phi_k2_part, sum_over_k2 );
	}
	
	cross_product_from_VSH_coeffs_20a = vector_sum( cross_product_from_VSH_coeffs_20a, cross_product_from_VSH_coeffs_20a_Phi_k1_part );
	cross_product_from_VSH_coeffs_20a = vector_sum( cross_product_from_VSH_coeffs_20a, cross_product_from_VSH_coeffs_20a_Phi_k2_part );*/
	...
	int w = 20;
	print_vector_setw( cross_product_from_VSH_coeffs_11 , w, "Psi_1 x Psi_2 (exact)" );
	print_vector_setw( cross_product_from_VSH_coeffs_11a, w, "Psi_1 x Psi_2 (added)" );
