...
// g++ Dot_product_of_VSH_series_103.cpp -o Dot_product_of_VSH_series_103
...
Use that the the dot produts over Phi are equal to those over Psi, and the only remaining mixed dot products are negatives of each other.
*/
...
int main(){
	...
	for( int k1=0; k1<=k_max; k1++ ){
		...
		for( int k2=0; k2<=k_max; k2++ ){	
			...
			dot_product_from_VSH_coeffs += Ar12[0] * Br12[0] * dot_product( Y1  , Y2   );
			dot_product_from_VSH_coeffs += ( Ar12[1] * Br12[1] + Ar12[2] * Br12[2] ) * dot_product( Psi1, Psi2 );
			dot_product_from_VSH_coeffs += ( Ar12[1] * Br12[2] - Ar12[2] * Br12[1] ) * dot_product( Psi1, Phi2 );
