...
// g++ 20221206_Check_dphi_Y_ell_minusm.cpp -o 20221206_Check_dphi_Y_ell_minusm

/*
Check the following relations:
d/dphi Y_ell^{-m} = (-1)^{m+1} e^{-i2mphi} d/dphi Y_ell^m
                  = (-1)^{m+1} e^{-i2mphi} im Y_ell^m.
*/
...
int main(){
	...
	for( int ell=0; ell<=ell_max; ell++ ){
		for( int m=0; m<=ell; m++ ){
			std::complex<double> SH_exact = ylm_dphi( theta_list[i_test], phi_list[j_test], ell, -m );
			std::complex<double> SH_test1 = pow(-1,m+1) * exp(std::complex<double>{0,-2*m*phi_list[j_test]}) * ylm_dphi( theta_list[i_test], phi_list[j_test], ell, m );
			std::complex<double> SH_test2 = pow(-1,m+1) * exp(std::complex<double>{0,-2*m*phi_list[j_test]}) * std::complex<double>{0,(double)m} * ylm( theta_list[i_test], phi_list[j_test], ell, m );
	...
	for( int ell=0; ell<=ell_max; ell++ ){
		for( int m=0; m<=ell; m++ ){
			...
			for( int i=0; i<N_theta; i++ ){
				for( int j=0; j<N_phi; j++ ){
					std::complex<double> SH_exact = ylm_dphi( theta_list[i], phi_list[j], ell, -m );
					std::complex<double> SH_test1 = pow(-1,m+1) * exp(std::complex<double>{0,-2*m*phi_list[j]}) * ylm_dphi( theta_list[i], phi_list[j], ell, m );
					std::complex<double> SH_test2 = pow(-1,m+1) * exp(std::complex<double>{0,-2*m*phi_list[j]}) * std::complex<double>{0,(double)m} * ylm( theta_list[i], phi_list[j], ell, m );
