...
// g++ Test_fraction_of_gamma_functions_2.cpp -o Test_fraction_of_gamma_functions_2

// Gamma( n - 1/2 ) / ( 2 Gamma( -1/2) ) = - 1 / (n-1/2) * (2n)! / ( 2^(2n+2) n! )
...
double RHS( int n ){
	return tgamma(2*n+1) / ( pow( 2, 2*n+2 ) * tgamma( n+1 ) ) * -1.0 / ( n - 0.5 );
}