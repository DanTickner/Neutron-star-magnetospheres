...
// g++ 20221206_Check_dtheta_Y_ell_minusm.cpp -o 20221206_Check_dtheta_Y_ell_minusm

/*
Check the following relation:
d/dtheta Y_ell^{-m} = (-1)^m e^{-i2mphi} d/dtheta Y_ell^m.
*/
...
int main(){
	...
	for( int ell=0; ell<=ell_max; ell++ ){
		for( int m=0; m<=ell; m++ ){
			std::complex<double> SH_exact = ylm_dtheta( theta_list[i_test], phi_list[j_test], ell, -m );
			std::complex<double> SH_test  = pow(-1,m) * exp(std::complex<double>{0,-2*m*phi_list[j_test]}) * ylm_dtheta( theta_list[i_test], phi_list[j_test], ell, m );
	...
	for( int ell=0; ell<=ell_max; ell++ ){
		for( int m=0; m<=ell; m++ ){
			...
			for( int i=0; i<N_theta; i++ ){
				for( int j=0; j<N_phi; j++ ){
					std::complex<double> SH_exact = ylm_dtheta( theta_list[i], phi_list[j], ell, -m );
					std::complex<double> SH_test  = pow(-1,m) * exp(std::complex<double>{0,-2*m*phi_list[j]}) * ylm_dtheta( theta_list[i], phi_list[j], ell, m );
