...
// g++ 20221206_Check_dtheta_Y_ell_m_star.cpp -o 20221206_Check_dtheta_Y_ell_m_star

/*
Check the following relation:
d/dtheta [Y_ell^m]^* = e^{-i2mphi} d/dtheta Y_ell^m.
*/
...
int main(){
	...
	for( int ell=0; ell<=ell_max; ell++ ){
		for( int m=-ell; m<=ell; m++ ){
			...
			std::complex<double> SH_test  = exp(std::complex<double>{0,-2*m*phi_list[j_test]}) * ylm_dtheta( theta_list[i_test], phi_list[j_test], ell, m );
		...
		for( int m=-ell; m<=ell; m++ ){
			...
			for( int i=0; i<N_theta; i++ ){
				for( int j=0; j<N_phi; j++ ){
					...
					std::complex<double> SH_test  = exp(std::complex<double>{0,-2*m*phi_list[j]}) * ylm_dtheta( theta_list[i], phi_list[j], ell, m );
