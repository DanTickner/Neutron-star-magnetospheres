...
// g++ sum_of_chebyshevs_and_spherical_harmonics_check_stdev.cpp -o sum_of_chebyshevs_and_spherical_harmonics_check_stdev
...
// This code calculates the stdev over a list of coordinates to check that my stdev calculation is correct. Answer should be zero.
...
std::complex<double> evaluate_3D_function( double r, double t, double p, std::vector< std::vector< std::vector< std::complex<double> > > > coeffs );
...
int main(){
	...
	int N_r     = 121;		// Number of integration terms in the r direction.
	int N_t     = 120;		// Number of integration terms in the t direction.
	int N_p     = 120;		// Number of integration terms in the p direction.
	...
	std::vector<double> p( N_p );
	
	//----- Build lists of coordinates -----
	generate_coeffs_chebyshev( n_max );				// Only needed for f_function().
	generate_coeffs_spherical_harmonic( ell_max );	// Needed for the fitting method and for f_function. In later versions, it will only be needed for f_function().
	
	for( int i=0; i<N_r; i++ ){
		r[i] = -1 + 2.0 / ( (double) N_r - 1 ) * i;
	}
	for( int j=0; j<N_t; j++ ){
		t[j] = pi / ( (double) N_t - 1 ) * j;
	}
	for( int k=0; k<N_p; k++ ){
		p[k] = 2*pi / ( (double) N_p - 1 ) * k;
	}
	
	//----- Generate coefficients and b -----
	...
	for( int n=0; n<=n_max; n++ ){
		std::vector< std::vector< std::complex<double> > > coeffs_for_this_n;
		for( int ell=0; ell<=ell_max; ell++ ){
			std::vector< std::complex<double> > coeffs_for_this_ell;
			for( int m=-ell; m<=ell; m++ ){
				coeffs_for_this_ell.push_back( std::complex<double> { a, b } );
				a += da;
				b += db;
			}
			coeffs_for_this_n.push_back( coeffs_for_this_ell );
		}
		coeffs.push_back( coeffs_for_this_n );
	}
	
	for( int n=0; n<=n_max; n++ ){
		for( int ell=0; ell<=ell_max; ell++ ){
			for( int m=-ell; m<=ell; m++ ){
				std::cout <<"("<< n <<","<< ell <<","<< m <<"):\t" << coeffs[n][ell][ell+m] << std::endl;
			}
		}
	}
	...
	for( int i=0; i<N_r; i++ ){
		for( int j=0; j<N_t; j++ ){
			for( int k=0; k<N_p; k++ ){
				stdev += pow( f(r[i],t[j],p[k],n_max,ell_max) - evaluate_3D_function(r[i],t[j],p[k],coeffs), 2 );
			}
		}
	}
	stdev = sqrt( stdev / ( (double) N_r * N_t * N_p ) );
...
std::complex<double> evaluate_3D_function( double r, double t, double p, std::vector< std::vector< std::vector< std::complex<double> > > > coeffs ){
	...
	for( int n=0; n<coeffs.size(); n++ ){
		double pn = polynomial_chebyshev_1( r, n );
		for( int ell=0; ell<coeffs[n].size(); ell++ ){
			ret += coeffs[n][ell][ell] * pn * ylm( t, p, ell, 0 );
			for( int m=1; m<=ell; m++ ){
				ret += ( coeffs[n][ell][ell-m] * exp(std::complex<double>{0,-2.0*m*p}) * pow(-1,m) + coeffs[n][ell][ell+m] ) * pn * ylm( t, p, ell, m );
			}
		}
	}
	return ret;
...
std::complex<double> f( double r, double t, double p, int n_max, int ell_max ){
	...
	for( int n=0; n<=n_max; n++ ){
		for( int ell=0; ell<=ell_max; ell++ ){
			for( int m=-ell; m<=ell; m++ ){
				//std::cout <<"("<< a <<"+"<< b <<"i) * T_"<< n <<" * Y_"<< ell <<"^"<< m << std::endl;		// Comment-out if taking stdev!
	...
	//std::cout << "Coeff total:\t" << std::complex<double>{a_tot,b_tot} << std::endl;						// Comment-out if taking stdev!
	return ret;
}
