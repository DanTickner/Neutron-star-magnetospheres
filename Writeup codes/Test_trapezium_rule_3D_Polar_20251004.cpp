...
g++ Test_trapezium_rule_3D_Polar_20251004.cpp -o Test_trapezium_rule_3D_Polar_20251004 --std=c++11
Test_trapezium_rule_3D_Polar_20251004

Test the 3D trapezium rule for a known integral in Polar coordinates.
...
int main(){
	...
	//----- Define variables -----
	std::string output_filename_csv = "CSV/20251004_Test_trapezium_rule_3D_Polar.csv";
		...
		for( int N2=0; N2<N_values_u2_vector.size(); N2++ ){
			for( int N3=0; N3<N_values_u3_vector.size(); N3++ ){
				...
				for( int i=0; i<N_values_u1; i++ ){
					for( int j=0; j<N_values_u2; j++ ){
						for( int k=0; k<N_values_u3; k++ ){
							dV[i][j][k] = (1.0/3.0) * ( pow(u1[i+1],3) - pow(u1[i],3) ) * ( cos(u2[j]) - cos(u2[j+1]) ) * ( u3[k+1] - u3[k] );		// This will need to change for different coordinate systems.
						}
					}
				}
...
//----- Define functions -----
double f_function( double u1, double u2, double u3 ){
	return u1 * cos( u2 ) * sin( u3 );
}

double integral_indefinite_function( double u1, double u2, double u3 ){
	return 0.0625 * pow(u1,4) * cos( 2.0 * u2 ) * cos( u3 );
}