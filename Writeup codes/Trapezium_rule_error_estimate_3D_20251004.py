'''
Trapezium_rule_error_estimate_3D_20251004
...
#csv_filename = "CSV/20251004_Test_trapezium_rule_3D_Cartesian.csv"
csv_filename = "CSV/20251004_Test_trapezium_rule_3D_Polar.csv"
...
with open( csv_filename ) as csv_file:
	...
	for row in data:
		...
		N_points_3.append( int  ( row[2] ) )
		abs_err   .append( float( row[4] ) )


#----- Manually perform the power-law fit -----
def func( N_tuple, A1, k1, A2, k2, A3, k3 ):
	# The fitting function is probably the only thing to change between Cartesian and polar.
	N1, N2, N3 = N_tuple
	return A1 * N1**k1 + A2 * N2**k2 + A3 * N3**k3
...
for i in range(len( N_points_1 )):
	...
	err += popt[4] * N_points_3[i]**popt[5]
	abs_err_fit.append( err )
	abs_err_fit_rel_diff.append( abs( ( err - abs_err[i] ) / abs_err[i] ) )

#----- Output results -----
for i in range( len( N_points_1 ) ):
	print( f"{ N_points_1[i] }\t{ N_points_2[i] }\t{ N_points_3[i] }\t{ abs_err[i] }\t{ abs_err_fit[i] }\t{ abs_err_fit_rel_diff }" )
