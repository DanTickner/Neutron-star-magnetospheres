...
g++ VSH_series_03.cpp -o VSH_series_03 --std=c++11
VSH_series_03
...
V03: Just do the for-loop from ell=0 without considering ell=0 separately.
     It means that A^{(1),0}_0 = A^{(2),0}_0 = nan, but this is only due to the 1/(ell(ell+1)) division; there is no other real issue. So just set these coeffs back to zero afterward.
	 This makes everything far simpler, especially for codes based off this which calculate e.g. fAxB.
