...
g++ AVSH_series_fA_01.cpp -o AVSH_series_fA_01 --std=c++11
AVSH_series_fA_01
...
INCORRECT: We can't calculate everything in the same loop over \ell because the coeffs of fA require us to ALREADY HAVE all the coeffs of A.
...
int main(){
	...
	std::cout << "----- This code is incorrect! Use at least V02 instead! -----\n" << std::endl;
	
	//----- Define variables -----
	int ell_max = 2;
	int n_steps_integration = 1e4;
	int w = 20;							// Fixed width for screen output.
	...
	for( int ell=0; ell<=ell_max; ell++ ){
		...
		for( int L1=0; L1<=ell_max; L1++ ){
			...
			if( ell == 0 ){
				//std::cout << "ell=0, L1=" <<L1 <<"\t"<< coeffs_fA_r_from_A[ell] <<"\t"<< integral_r <<"\t"<< coeffs_A_r[L1] <<"\t"<< sqrt( 2.0*L1+1.0 ) << std::endl;
				std::cout << L1 <<"\t"<< coeffs_A_r[L1] << std::endl;
			}
		...
		//std::cout << ell << "\t"<< 0 <<"\t|\t"<< std::left<<std::setw(w)<< coeffs_fA_r[ell] <<std::left<<std::setw(w)<< coeffs_fA_r_from_A[ell] << std::endl;
	}
	
	
	//--- Check: Calculate [fA]^{r,0} again, as it seems to cause issues. ---
	...
	for( int L1=0; L1<=ell_max; L1++ ){
		...
		for( int i=0; i<n_steps_integration; i++ ){
			...
			integrand_values_fA_r_from_A[i] = f * P_L1_0 * sin( theta );
			
		}
		
		fA_r_0 += trapezium( integrand_values_fA_r_from_A, theta_list ) * coeffs_A_r[L1] * sqrt( 2.0*L1+1.0 );
		
	}
	
	fA_r_0 *= 0.5;
	
	std::cout << "fA_r_0 = " << fA_r_0 << std::endl;
	...
	for( int ell=0; ell<=ell_max; ell++ ){
		...
		std::cout << ell <<"\t"<< coeffs_fA_r[ell] <<"\t"<< coeffs_fA_r_from_A[ell] <<"\t|\t"<< fA_series[0] <<"\t"<< fA_series_from_A[0] << std::endl;
