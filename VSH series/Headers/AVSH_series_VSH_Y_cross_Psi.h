...
double A_phi_function( double r, double theta ){
	//return sqrt( (2*L1+1) * (2*L2+1) ) / ( 4*pi ) * p_ell( cos(theta), L1 ) * p_ell_1( cos(theta), L2 );
	generate_coeffs_spherical_harmonic( std::max( L1, L2 ) );
	return cross_product( VSH_Y( theta, 0, L1, 0 ), VSH_Psi( theta, 0, L2, 0 ) )[2].real();
}
...
double A_2_ell_guess( double r, int L ){
		...
		if( (L1+L2+L12)%2 == 0 ){
			...
			if( (L12+L)%2 == 0 ){
				for( int L123=std::max(abs(L12-L),2); L123<=L12+L; L123++ ){
					if( L123 %2 == 0 ){
						double G123          = ( 2*L123+1 ) * wigner_3j_000( L12, L, L123 ) * wigner_3j( L12, L, L123, 1, 1, -2 );
						double sqrt_fraction = pow( (L123+2) * (L123+1) * L123 * (L123-1), -0.5 );
	...
	return ret * sqrt( (2*L+1)*(2*L1+1)*(2*L2+1) * L2*(L2+1) / ((double) L*(L+1) * pi ) );
	*/
	...
	return I_011( L1, L2, L ) / ( (double) L*(L+1) );
}