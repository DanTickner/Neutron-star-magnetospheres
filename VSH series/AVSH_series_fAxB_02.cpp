...
g++ AVSH_series_fAxB_02.cpp -o AVSH_series_fAxB_02 --std=c++11
AVSH_series_fAxB_02
...
int main(){
	...
	for( int ell=0; ell<=ell_max; ell++ ){	
		...
		coeffs_fAxB_r_from_AB[ell] *= 0.25 * sqrt( ( 2.0*ell+1.0 ) / pi );
		coeffs_fAxB_1_from_AB[ell] *= 0.25 * sqrt( ( 2.0*ell+1.0 ) / pi ) / ( (double) ell*(ell+1.0) );
		coeffs_fAxB_2_from_AB[ell] *= 0.25 * sqrt( ( 2.0*ell+1.0 ) / pi ) / ( (double) ell*(ell+1.0) );
