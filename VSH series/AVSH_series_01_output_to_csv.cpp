...
g++ AVSH_series_01_output_to_csv.cpp -o AVSH_series_01_output_to_csv --std=c++11
AVSH_series_01_output_to_csv
...
Output the list of generated function values and datapoints to CSV, to aid in development of AVSH_series_from_CSV_01.cpp.
This won't have a use in general, so relegate to a new version which probably won't be used again.
...
int main(){
	...
	int p = 15;							// Desired precision of CSV output.
	...
	std::string output_filename = "CSV/20231206_dipole.csv";
	
	
	
	
	//----- Prepare output CSV and output headers -----
	output_file.open( output_filename );
	
	output_file << "theta,A_r,A_theta,A_phi\n";
	...
	for( int i=0; i<n_steps_integration; i++ ){
		...
		output_file << std::setprecision( p ) << theta_list[i] <<","<< A_r_values[i] <<","<< A_theta_values[i] <<","<< A_phi_values[i] << "\n";
	...
	std::cout << "CSV file saved:\t" << output_filename << std::endl;
