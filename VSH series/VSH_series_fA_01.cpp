...
g++ VSH_series_fA_01.cpp -o VSH_series_fA_01 --std=c++11
VSH_series_fA_01
...
int main(){
	...
	coeffs_A_r[0][0+0] = integral_spherical_surface( integrand_values_A_r, theta_list, phi_list );
	
	coeffs_fA_r_numerical[0][0+0] = integral_spherical_surface( integrand_values_fA_r, theta_list, phi_list );
	
	std::cout << 0 << "\t"<< 0 <<"\t|\t"<< std::left<<std::setw(w)<< coeffs_A_r[0][0+0] <<std::left<<std::setw(w)<< coeffs_A_1[0][0+0] <<std::left<<std::setw(w)<< coeffs_A_2[0][0+0] <<"\n"<< std::endl;
		...
		for( int M1=-L1; M1<=L1; M1++ ){
			...
			for( int i=0; i<n_steps_integration_theta; i++ ){
				for( int j=0; j<n_steps_integration_phi; j++ ){
					...
					std::complex<double> ylm_conj = std::conj( ylm_values[i][j][0][0+0] );
			...
			coeffs_fA_r_series[0][0+0] += integral_spherical_surface( integrand_values_fA_r, theta_list, phi_list );
		}
	}
	
	//std::cout << coeffs_fA_r_numerical[0][0+0] <<"\t"<< coeffs_fA_r_series[0][0+0] << std::endl;
