...
g++ VSH_series_04.cpp -o VSH_series_04 --std=c++11
VSH_series_04
...
V04: Change output to have calculated and guessed coeffs alongside each other, cf. AVSH_series_01.cpp.
...
int main(){
	...
	//----- Define variables -----
	int ell_max = 3;
	int n_steps_integration_theta = 1e4;
	int n_steps_integration_phi   = 1e1;
	int w = 28;							// Fixed width for screen output.
	...
	for( int ell=0; ell<=ell_max; ell++ ){
		for( int m=-ell; m<=ell; m++ ){
			...
			std::cout << ell << "\t"<< m <<"\t| "<< std::left<<std::setw(w)<< coeffs_r[ell][ell+m] <<std::left<<std::setw(w)<< A_r_ell_guess(test_r,ell,m) <<" | "<<std::left<<std::setw(w)<< coeffs_1[ell][ell+m] <<std::left<<std::setw(w)<< A_1_ell_guess(test_r,ell,m) <<" | "<< std::left<<std::setw(w)<< coeffs_2[ell][ell+m] <<std::left<<std::setw(w)<< A_2_ell_guess(test_r,ell,m) << std::endl;
		}
		std::cout << std::endl;
	}
